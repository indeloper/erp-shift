...
trait DevExtremeDataSourceLoadable
{
     ...
     */
    protected function appendSearchOperation(
        $query,
     ...
     *   С группой условий:
     * "filter":[["id","<>",35],"and",["operation_route_id","=",2]] Со
     * вложенными условиями:
     * "filter":[["id","<>",35],"and",[["date_start",">=","2020/11/03
     * 00:00:00"],"and",["date_start","<=","2020/11/04 00:00:00"]]]
     *
     * @return mixed
     */
    protected function appendFilter(
        ...
        $isFilterConditionSimple = ! is_array($filterArray[0])
            && count($filterArray) == 3;
        ...
        if ($isFilterConditionSimple) {
            ...
            if ($useHaving) {
                $result->having(
                    $this->formatField($translatedFilterItem['fieldName'],
                        $translatedFilterItem['operator']),
                    $this->conditionOperators[$translatedFilterItem['operator']],
                    $this->formatValue($translatedFilterItem['value'],
                        $translatedFilterItem['operator'])
                );
            } else {
                $result->where(
                ...
                if (is_array($filterItem)) {
                    if ($useHaving) {
                        $this->appendFilter($query, $filterItem,
                            $logicalOperator, $useHaving);
                    } else {
                        switch ($logicalOperator) {
                            case 'and':
                                $result->where(function ($query) use (
                                    $filterItem,
                                    $logicalOperator,
                                    $useHaving
                                ) {
                                ...
                                break;
                            case 'or':
                                $result->orWhere(function ($query) use (
                                    $filterItem,
                                    $logicalOperator,
                                    $useHaving
                                ) {
                    ...
                    if (isset($this->logicalOperators[$filterItem])) {
    ...
    /**
     * Добавляет в запрос условия, генерируемые в loadOptions метода load
     * класса CustomDataStore компонент DevExtreme
     *
     * @return Builder|static
     */
    public function dxLoadOptions(
        ?object $loadOption,
        $useHavingInsteadOfWhere = false
    ) {
        $result = $this::query();
        ...
        if (isset($loadOption->searchOperation)
            && ($loadOption->searchValue != null)
        ) {
            $this->appendSearchOperation($result, $loadOption->searchOperation,
                $loadOption->searchValue, $loadOption->searchExpr);
        }
