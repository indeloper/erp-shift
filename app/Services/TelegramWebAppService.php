...
class TelegramWebAppService
{
    ...
    /**
     * A key for hashing a key that will be used to calculate the hash from the
     * data received via Telegram MiniApp
     */
    private const SHA256_TOKEN_HASH_KEY = 'WebAppData';

    /**
     * Default
     * {@link https://core.telegram.org/bots/webapps#webappinitdata Telegram initData} auth_date lifetime
     * (seconds)
     */
    ...
    /**
     * Validate data received via the Mini App, one should send the data from
     * the Telegram.WebApp.initData field. The data is a query string, which is
    ...
    public function verifyInitDataArray(array $queryParams = []): bool
    {
        if ( ! $this->telegramInitDataValid($queryParams)) {
            return false;
        }
    ...
    public function getWebAppUser(?Request $request = null): ?TelegramUser
    {
        $requestQuery = ! $request
            ? \Illuminate\Support\Facades\Request::query() : $request->query();
        if ( ! array_key_exists(self::USER_QUERY_PARAMETER_KEY,
            $requestQuery)
        ) {
            return null;
        }
        $telegramUserData
            = json_decode($requestQuery[self::USER_QUERY_PARAMETER_KEY],
    ...
    /**
     * Verify integrity of the data received by comparing the received hash
     * parameter with the hexadecimal representation of the HMAC-SHA-256
     * signature of the data-check-string with the secret key, which is the
     * HMAC-SHA-256 signature of the bot's token with the constant string
     * WebAppData used as a key.
     */
    private function createHashFromQueryString(array $queryParams): string
    {
        $dataDigestKey   = $this->crypto->hmacSHA256(telegramToken(),
            self::SHA256_TOKEN_HASH_KEY, true);
    ...
    /**
     * Checking that
     * {@link https://core.telegram.org/bots/webapps#webappinitdata Telegram initData} is valid
     */
    private function telegramInitDataValid(array $telegramInitData): bool
    {
        return array_key_exists(self::USER_QUERY_PARAMETER_KEY,
                $telegramInitData)
            && ! $this->authDateExpired($telegramInitData[self::AUTH_DATE_QUERY_PARAMETER_KEY]);
    }

    /**
     * Checking that Telegram
     * {@link https://core.telegram.org/bots/webapps#webappinitdata auth_date}
     * parameter has expired The lifetime of
     * {@link https://core.telegram.org/bots/webapps#webappinitdata Telegram initData} is set by the telegram-webapp.authDateLifetimeSeconds parameter
     */
    private function authDateExpired(int $authDate): bool
    {
        $authDateLifetime = webAppConfig('authDateLifetimeSeconds',
            self::DEFAULT_AUTH_DATE_LIFETIME);
