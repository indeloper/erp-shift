 ...
 *  define:
 *      C_REST_WEB_HOOK_URL =
 *     'https://rest-api.bitrix24.com/rest/1/doutwqkjxgc3mgc1/'  //url on creat
 *     Webhook or C_REST_CLIENT_ID = 'local.5c8bb1b0891cf2.87252039'
 *     //Application ID C_REST_CLIENT_SECRET =
 *     'SakeVG5mbRdcQet45UUrt6q72AMTo7fkwXSO7Y5LYFYNCRsA6f'//Application key
 *
 *        C_REST_CURRENT_ENCODING = 'windows-1251'//set current encoding site
 *     if encoding unequal UTF-8 to use iconv() C_REST_BLOCK_LOG = true //turn
 *     off default logs C_REST_LOGS_DIR = __DIR__ .'/logs/' //directory path to
 *     save the log C_REST_LOG_TYPE_DUMP = true //logs save var_export for
 *     viewing convenience C_REST_IGNORE_SSL = true //turn off validate ssl by
 *     curl
 */
class CRest
{
    ...
    public static function installApp()
    {
        ...
        if ($_REQUEST['event'] == 'ONAPPINSTALL'
            ...
            $result['install'] = static::setAppSettings($_REQUEST['auth'],
                true);
    ...
    protected static function callCurl($arParams)
    {
        ...
        if ($arSettings !== false) {
            if (isset($arParams['this_auth'])
                && $arParams['this_auth'] == 'Y'
            ) {
                $url = 'https://oauth.bitrix.info/oauth/token/';
            } else {
                $url = $arSettings["client_endpoint"].$arParams['method'].'.'
                    .static::TYPE_TRANSPORT;
                if (empty($arSettings['is_web_hook'])
     ...
     *          'params' => [ "entity_type" => "CONTACT",  "type" => "EMAIL",
     *     "values" => array("info@bitrix24.com") ]
     *      ],
     *      'get_contact' => [
     ...
     *          'params' => [ "id" => '$result[get_contact][COMPANY_ID]',
     *     "select" => ["*"],]
     *      ]
     * ];
     *
    ...
    protected static function getSettingData()
    {
        ...
        if (file_exists(__DIR__.'/settings.json')) {
            $return = static::expandData(file_get_contents(__DIR__
                .'/settings.json'));
            if (defined("C_REST_CLIENT_ID") && ! empty(C_REST_CLIENT_ID)) {
                $return['C_REST_CLIENT_ID'] = C_REST_CLIENT_ID;
            }
            if (defined("C_REST_CLIENT_SECRET")
                && ! empty(C_REST_CLIENT_SECRET)
            ) {
    ...
    protected static function changeEncoding($data, $encoding = true)
    {
        if (is_array($data)) {
            ...
            if ($encoding) {
                $result = iconv(C_REST_CURRENT_ENCODING, "UTF-8//TRANSLIT",
                    $data);
            } else {
    ...
    {
        return (boolean) file_put_contents(__DIR__.'/settings.json',
    ...
    public static function setLog($arData, $type = '')
    {
        ...
        if ( ! defined("C_REST_BLOCK_LOG") || C_REST_BLOCK_LOG !== true) {
            ...
            if ( ! defined("C_REST_LOG_TYPE_DUMP")
                || C_REST_LOG_TYPE_DUMP !== true
            ) {
                $jsonLog = static::wrapData($arData);
                if ($jsonLog === false) {
                    $return = file_put_contents($path.'_backup.txt',
    ...
    public static function checkServer($print = true)
    {
        ...
        //creat setting file
        file_put_contents(__DIR__.'/settings_check.json',
        ...
        if ( ! mkdir($path, 0775, true) && ! file_exists($path)) {
            ...
            file_put_contents($path.'test.txt',
