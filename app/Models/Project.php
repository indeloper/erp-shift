...
class Project extends Model
{
    ...
    public function getTongueStatusesAttribute()
    {
        ...
        $contracts = $contracts->filter(function ($item, $key) use ($com_offers) {
            return $item->commercial_offers->count() && $item->commercial_offers->whereNotIn('option', $com_offers)->where('is_tongue', 1)->count();
        });
        $contracts_complete = $this->contracts->whereNotIn('id', $contracts->pluck('id'));

        $contracts_complete = $contracts_complete->filter(function ($item, $key) use ($com_offers) {
            return $item->commercial_offers->count() && $item->commercial_offers->whereNotIn('option', $com_offers)->where('is_tongue', 1)->count();
    ...
    public function getPileStatusesAttribute()
    {
        ...
        $contracts = $contracts->filter(function ($item, $key) use ($com_offers) {
            return $item->commercial_offers->count() && $item->commercial_offers->whereNotIn('option', $com_offers)->where('is_tongue', 0)->count();
        });
    ...
    public function in_not_realized($option, $is_tongue)
    {
        ...
        $all_wvs_declined = $this->wvs->where('type', $type)->where('option', $option)->count() == $this->wvs->where('type', $type)->where('option', $option)->where('status', 3)->count();
        $all_com_offers_declined = ($this->com_offers->where('is_tongue', $is_tongue)->where('option', $option)->count() == $this->com_offers->where('is_tongue', $is_tongue)->where('option', $option)->where('status', 3)->count()) && $this->com_offers->where('is_tongue', $is_tongue)->where('option', $option)->count() != 0;
