...
class ProjectCommercialOfferController extends Controller
{
    ...
    {
        $commercial_offer = CommercialOffer::with('notes', 'requirements',
            'advancements', 'project')
            ->findOrFail($com_offer_id);

        $work_volume = WorkVolume::with('works_offer.materials',
            'works_offer.manual')
            ->where('work_volumes.id', $commercial_offer->work_volume_id)
            ->leftJoin('projects', 'projects.id', '=',
                'work_volumes.project_id')
        ...
        foreach ($works as $work) {
            $materials_ids[] = $work->relations->pluck('wv_material_id')
                ->toArray();
        }
        ...
        $commercial_offer_requests = CommercialOfferRequest::where('project_id',
            $project_id)
            ->where('commercial_offer_requests.commercial_offer_id',
                $commercial_offer->id)
            ->leftJoin('users', 'users.id', '=',
                'commercial_offer_requests.user_id')
            ->select('commercial_offer_requests.*', 'users.last_name',
                'users.first_name', 'users.patronymic')
            ->with('files');

        $splits = $commercial_offer->mat_splits()->with([
            'subcontractor_file', 'buyback', 'security',
        ])->get();

        //control splits material type
        foreach ($splits as $split) {
            $wv_mat = $work_volume->materials->where('manual_material_id',
                $split->man_mat_id)->first();
            if ($wv_mat) {
        ...
        $subcontractors = Contractor::with([
            'file' => function ($q) use ($works_files, $commercial_offer) {
                $q->where('type', 0)->whereIn('id', $works_files)
                    ->whereIn('commercial_offer_id',
                        $commercial_offer->siblings->pluck('id'));
            },
        ])->whereHas('file',
            function ($q) use ($works_files, $commercial_offer) {
                $q->whereIn('id', $works_files)->whereIn('commercial_offer_id',
                    $commercial_offer->siblings->pluck('id'));
            })->get();

        $material_subcontractors = Contractor::with([
            'file' => function ($q) use ($materials_files, $commercial_offer) {
                $q->where('type', 1)->whereIn('id', $materials_files)
                    ->whereIn('commercial_offer_id',
                        $commercial_offer->siblings->pluck('id'));
            },
        ])->whereHas('file',
            function ($q) use ($materials_files, $commercial_offer) {
                $q->whereIn('id', $materials_files)
                    ->whereIn('commercial_offer_id',
                        $commercial_offer->siblings->pluck('id'));
            })->get();

        $resp_con = ProjectResponsibleUser::where('project_id', $project_id)
            ->where('role', 2)->first();

        $work_volumes = WorkVolume::where('project_id', $project_id)
            ->where('type', '!=', 2)
            ->orderBy('work_volumes.version', 'desc')
            ->with('get_requests');

        $agree_task = Task::where('project_id', $project_id)
            ->where('status', 16)->where('target_id', $com_offer_id)
            ->where('is_solved', 0)->where('responsible_user_id', Auth::id())
            ->first();
        ...
        $com_offers_options = CommercialOffer::where('project_id', $project_id)
            ...
            ->select('commercial_offers.*', DB::raw('max(version) as version'),
                DB::raw('max(id) as id'))
            ->get();
        ...
        return view('projects.commercial_offer.card', [
            ...
            'work_volume_materials'     => $work_volume_materials->unique('manual_material_id')
                ->sortBy('work_group_id'),
    ...
    public function card_pile($project_id, $com_offer_id): View
    {
        ...
        $work_volume = WorkVolume::where('work_volumes.id',
        ...
        $splits
            = CommercialOfferMaterialSplit::with('WV_material.manual.category')
            ->where('commercial_offer_material_splits.com_offer_id',
                $com_offer_id)
            ...
            ->leftjoin('contractors', 'contractors.id', '=',
                'contractor_files.contractor_id')
            ->select('commercial_offer_material_splits.*',
                'contractors.short_name')
            ->get();

        $works_files     = $commercial_offer->works->where('subcontractor_file_id',
            '!=', null)->pluck('subcontractor_file_id')->unique();
        $materials_files = $splits->where('subcontractor_file_id', '!=', null)
            ->pluck('subcontractor_file_id')->unique();
    ...
    public function card_double($project_id, $com_offer_id): View
    {
        ...
        $manual_notes
            = CommercialOfferManualNote::where('commercial_offer_type', '=',
            $commercial_offer->is_tongue ? 1 : 2)->get();
        $manual_requirements
            = CommercialOfferManualRequirement::where('commercial_offer_type',
            '=', $commercial_offer->is_tongue ? 1 : 2)->get();
    ...
    public function edit($project_id, $com_offer_id)
    {
        ...
        if ($commercial_offer->status != 1) {
            ...
            if (strpos(url()->previous(), 'common_task')) {
                $commercial_offer->unsolved_tasks->where('status', 5)
                    ->each(function ($task) {
                        $task->solve();
                    });
            }
            ...
            return redirect()->route('projects::commercial_offer::card_'
                .($commercial_offer->is_tongue ? 'tongue' : 'pile'),
        ...
        $works_files     = $commercial_offer->works->pluck('subcontractor_file_id')
            ->unique();
        $materials_files = $splits->pluck('subcontractor_file')->unique();
        ...
        $signers        = collect([]);
        $signers_groups = Group::whereIn('id',
            [5/*3*/, 8/*5*/, 50/*7*/, 6/*24*/, 19/*33*/]);
        ...
        $title    = isset($commercial_offer->title)
            ? $commercial_offer->title
            : 'Коммерческое предложение на '.$work_volume->project_name.' «'
            .$object->name.'» по адресу: '.$object->address.
            (isset($object->cadastral_number)
                ? ', на земельном участке с кадастровым номером '
                .$object->cadastral_number : '');
        $contacts = ContractorContact::where('contractor_id',
            $project->contractor_id)->get();
    ...
    public function split_material(Request $request, $com_offer_id)
    {
        ...
        if ($request->has('target_split_id')) {
            $target_split
                = CommercialOfferMaterialSplit::find($request->target_split_id);
            $splitService->mergeRelatedSplitWithOldOne($old_split,
                $request->count, $target_split);
        } else {
            $splitService->splitMore($old_split, $request->count,
                $request->new_type, $request->time, $request->comment);
        }
    ...
    public function set_contract_number(
        Request $request,
        $project_id,
        $com_offer_id
    ) {
        ...
        if (in_array($request->field, ['contract_number', 'contract_date'])) {
            CommercialOffer::where('id', $com_offer_id)
    ...
    public function add_advancement(
        Request $request,
        $project_id,
        $com_offer_id
    ) {
        ...
        $avans_value = number_format(($request->avans_unit === '%'
            ? $request->avans_value / 100 * $request->max_avans
            : $request->avans_value), 2, ',', ' ');
        CommercialOfferAdvancement::create(
            [
                'commercial_offer_id' => $com_offer_id,
                'value'               => $request->avans_value
                    ? $request->avans_value : 0,
                'is_percent'          => $request->avans_unit === '%' ? 1 : 0,
                'description'         => trim(($request->avans_title
                    .' в размере '.$avans_value.' руб.'.($request->avans_note
                        ? ', '.$request->avans_note : ''))),
            ]
        );
    ...
    {
        $count = CommercialOfferAdvancement::where('id', $request->adv_id)
            ->update(['description' => trim($request->adv_desc)]);
    ...
    public function change_comment(Request $request, $com_offer_id)
    {
        if ($request->note_id > 0) {
            $count = CommercialOfferNote::where('id', $request->note_id)
    ...
    {
        if ($request->req_id > 0) {
            $count = CommercialOfferRequirement::where('id', $request->req_id)
                ->update(['requirement' => trim($request->req)]);

            if (trim($request->req) == '') {
                CommercialOfferRequirement::where('id', $request->req_id)
                    ->delete();
    ...
    public function request_store(Request $request, $project_id)
    {
        ...
        $offer_id = $request->add_tongue == 1 ? $request->tongue_offer_id
            : $request->pile_offer_id;
        $project  = Project::findOrFail($project_id);
        $offer    = CommercialOffer::find($offer_id);
        $SOP      = ProjectResponsibleUser::where('project_id', $project->id)
            ->where('role', 2)->first();

        if ($SOP) {
            if ($offer) {
                if (($offer->is_tongue == 1) or ($offer->is_tongue == 1
                        && $offer->status != 1)
                ) {
                    $offer->decline();
                    ...
                    $commercial_offer->name
                                                  = 'Коммерческое предложение (шпунтовое направление)';
                    $commercial_offer->user_id
                                                  = ProjectResponsibleUser::where('project_id',
                        $project->id)->where('role', 2)->first()->user_id;
                    $commercial_offer->project_id = $project->id;
                    $commercial_offer->work_volume_id
                                                  = WorkVolume::where('project_id',
                        $project->id)->where('status', 2)->where('type', 0)
                        ->where('option', $offer->option)->first()->id;
                    ...
                    if ($offer) {
                        ...
                        $splits
                            = CommercialOfferMaterialSplit::where('com_offer_id',
                            $offer->id)->get();

                        $remember_old_new_split
                            = []; //replication main split types
                        foreach (
                            $splits->where('parent_id', null) as $mat_split_old
                        ) {
                            $mat_split_copy = $mat_split_old->replicate();
                            $mat_split_copy->com_offer_id
                                            = $commercial_offer->id;
                            $mat_split_copy->man_mat_id
                                            = $mat_split_old->man_mat_id;

                            $subcontractor_file
                                = $mat_split_old->subcontractor_file;
                            if ($subcontractor_file) {
                                $file_copy = $subcontractor_file->replicate();
                                $file_copy->commercial_offer_id
                                           = $commercial_offer->id;
                                $file_copy->save();

                                $mat_split_copy->subcontractor_file_id
                                    = $file_copy->id;
                            }
                            $mat_split_copy->save();

                            $remember_old_new_split[$mat_split_old->id]
                                = $mat_split_copy->id;
                        }
                        // replicating children and updating parent_id
                        foreach (
                            $splits->where('parent_id', '!=', null) as
                            $mat_split_old
                        ) {
                            $mat_split_copy = $mat_split_old->replicate();
                            $mat_split_copy->com_offer_id
                            ...
                            $mat_split_copy->parent_id
                                = $remember_old_new_split[$mat_split_old->parent_id];
                            $subcontractor_file
                                = $mat_split_old->subcontractor_file;
                            if ($subcontractor_file) {
                                $file_copy = $subcontractor_file->replicate();
                        ...
                        if ($offer->commercial_offer_works()->count()) {
                            foreach ($offer->works as $work) {
                                ...
                                $new_work->commercial_offer_id
                                          = $commercial_offer->id;
                                $new_work->save();
                            }
                        } else {
                            foreach (
                                $commercial_offer->work_volume->works as $work
                            ) {
            ...
            if (isset($offer) && $offer->is_tongue == 1) {
                $thisTask = Task::where('project_id', $project_id)
                    ->where('status', 15)->where('is_solved', 0)->count();
                $task     = Task::where('project_id', $project->id)
                    ->where('status', 4)->where('target_id', $offer_id)
                ...
                if ($thisTask == 0) {
                    ...
                    $tongueTask->name
                                                     = 'Назначение ответственного за КП (шпунт)';
                    $tongueTask->status              = 15;
                    $tongueTask->responsible_user_id = Group::find(50/*7*/)
                        ->getUsers()->first()->id;
                    $tongueTask->contractor_id       = $project->contractor_id;
                    $tongueTask->expired_at          = Carbon::now()
                        ->addHours(3);
                    ...
                    AppointmentOfResponsibleForOfferSheetPilingTaskNotice::send(
                        ...
                        [
                            ...
                            'contractor_id'   => $tongueTask->project_id
                                ? Project::find($tongueTask->project_id)->contractor_id
                                : null,
                            'project_id'      => $tongueTask->project_id
                                ? $tongueTask->project_id : null,
                            'object_id'       => $tongueTask->project_id
                                ? Project::find($tongueTask->project_id)->object_id
                                : null,
                        ]
                    );
                }
        ...
        if (isset($commercial_offer) && $commercial_offer->is_tongue == 1) {
            ...
            $com_offer_request->description
                                                    = $request->descriptionTongue
                ? $request->descriptionTongue : $request->description;
            ...
            if ($request->tongue_documents) {
                foreach ($request->tongue_documents as $document) {
                    ...
                    $file_name = 'project-'.$com_offer_request->project_id
                        .'-com_offer'.$com_offer_request->commercial_offer_id
                        .'-request_file-'.uniqid().'.'.$mime;

                    Storage::disk('commercial_offer_request_files')
                        ->put($file_name, File::get($document));
            ...
            if ($request->project_documents_tongue) {
                $project_docs = ProjectDocument::whereIn('id',
                    $request->project_documents_tongue)->get();

                foreach ($request->project_documents_tongue as $document_id) {
                    $file = new CommercialOfferRequestFile();

                    $file->file_name     = $project_docs->where('id',
                        $document_id)->first()->file_name;
                    $file->request_id    = $com_offer_request->id;
                    $file->is_result     = 0;
                    $file->original_name = $project_docs->where('id',
                        $document_id)->first()->name;
                    $file->is_proj_doc   = 1;
        ...
        if ($offer) {
            if (($offer->is_tongue == 0)
                ...
                $commercial_offer->name
                                              = 'Коммерческое предложение (свайное направление)';
                $commercial_offer->user_id
                                              = ProjectResponsibleUser::where('project_id',
                    $project->id)->where('role', 1)->first()->user_id;
                $commercial_offer->project_id = $project->id;
                $commercial_offer->work_volume_id
                                              = WorkVolume::where('project_id',
                    $project->id)->where('status', 2)
                    ->where('option', $offer->option)->where('type', 1)
                    ->first()->id;
                $commercial_offer->status     = 1;
                $commercial_offer->version    = ($offer->version ?? 0) + 1;
                $commercial_offer->option     = $offer->option;
                ...
                $task = new Task([
                    ...
                    'responsible_user_id' => ProjectResponsibleUser::where('project_id',
                        $commercial_offer->project_id)->where('role', 1)
                        ->first()->user_id,
                    'contractor_id'       => $project->contractor_id,
                    'target_id'           => $commercial_offer->id,
                    'expired_at'          => Carbon::now()->addHours(24),
        ...
        if ($commercial_offer->is_tongue == 0) {
            ...
            $com_offer_request->commercial_offer_id = $commercial_offer->id;
            $com_offer_request->status              = 0;
            $com_offer_request->description         = $request->descriptionPile
                ? $request->descriptionPile : $request->description;
            ...
            if ($request->project_documents_pile) {
                $project_docs = ProjectDocument::whereIn('id',
                    $request->project_documents_pile)->get();
        ...
        if (isset($CO) and (($request->has('add_tongue')
                    && ! $request->has('add_pile'))
                or ($request->has('add_pile') && ! $request->has('add_tongue')))
        ) {
            $type                 = $request->has('add_tongue') ? 0 : 1;
            $com_offer_for_update = CommercialOffer::where('project_id',
                $project_id)->where('is_tongue', $type)->get()->last();
            if (isset($com_offer_for_update)) {
                ...
                $task = Task::where('project_id', $project->id)
                    ->where('status', 6)
                    ->where('target_id', $com_offer_for_update->id)->first();
    ...
    public function request_update(Request $request)
    {
        ...
        $com_offer_request
            = CommercialOfferRequest::findOrFail($request->offer_request_id);

        if (isset($request->status)) {
            $com_offer_request->status         = $request->status == 'confirm'
                ? 1 : 2;
        ...
        if ($request->project_documents) {
            $project_docs = ProjectDocument::whereIn('id',
                $request->project_documents)->get();

            foreach ($request->project_documents as $document_id) {
                $file = new CommercialOfferRequestFile();

                $file->file_name     = $project_docs->where('id', $document_id)
                    ->first()->file_name;
                $file->request_id    = $com_offer_request->id;
                $file->is_result     = 1;
                $file->original_name = $project_docs->where('id', $document_id)
                    ->first()->name;
                $file->is_proj_doc   = 1;
        ...
        OfferProcessingNotice::send(
            ...
            [
                'name'            => ('Пользователь '.$user.' '.
                    ($request->status == 'confirm' ? 'подтвердил(а) '
                        : 'отклонил(а) ').'заявку на редактирование КП '
                    .($com_offer_request->is_tongue ? 'шпунтового' : 'свайного')
                    .' направления версии '.$KP->version.' по проекту '
                    .Project::find($com_offer_request->project_id)->name),
                'additional_info' => "\r\nЗаказчик: ".$proj->contractor_name.
                    "\r\nНазвание объекта: ".$proj->object->name.
                    "\r\nАдрес объекта: ".$proj->object->address.
                    "\r\n".'Ссылка на проект: ',
                'url'             => route('projects::card',
                    $com_offer_request->project_id),
    ...
    public function attach_subcontractor(
        AddSubcontractorRequest $request,
        $wv_id
    ): RedirectResponse {
        DB::beginTransaction();
        ...
        if ($request->document) {
            ...
            $file_name = 'project-'
                .CommercialOffer::find($request->com_offer_id)->project_id
                .'-com_offers'.$request->commercial_offer_id.'-contractor_file-'
                .uniqid().'.'.$mime;

            Storage::disk('commercial_offers_contractor_files')
                ->put($file_name, File::get($request->document));
            ...
            if ($request->type === '1') {
                CommercialOfferMaterialSplit::whereIn('id',
                    $request->subcontractor_works)
                    ->update(['subcontractor_file_id' => $file->id]);
            } else {
                $commercial_offer->worksForToggling()
                    ->whereIn('id', $request->subcontractor_works)
                    ->update(['subcontractor_file_id' => $file->id]);
            }
        }
    ...
    public function set_material_price(Request $request): JsonResponse
    {
        ...
        $material
            = CommercialOfferMaterialSplit::findOrFail($request->split_id);
    ...
    {
        CommercialOffer::where('id', $request->com_offer)
            ->update(['nds' => $request->nds]);
    ...
    public function toggle_work_mat(Request $request)
    {
        if ($request->type === '1') {
            ...
            $commercialOffer->worksForToggling()->where('id', $request->id)
                ->update(['is_hidden' => DB::raw('NOT is_hidden')]);
        } elseif ($request->type === '0') {
            CommercialOfferMaterialSplit::where('id', $request->id)
                ->update(['is_hidden' => DB::raw('NOT is_hidden')]);
        }
    ...
    public function get_subcontractors(Request $request)
    {
        ...
        if ($request->q) {
            $contractors->where(function ($contractors) use ($request) {
                $contractors->where('full_name', 'like',
                    '%'.trim($request->q).'%')
        ...
        $results = [
            [
                'value' => '',
                'text'  => 'Показано '.($contractors_count < 3
                        ? $contractors_count : '3').' из '.$contractors_count
                    .' найденных',
            ],
        ];
    ...
    public function detach_subcontractors(Request $request)
    {
        ...
        if ($request->type === '1') {
            CommercialOfferMaterialSplit::where('subcontractor_file_id',
                ...
                ->update(['subcontractor_file_id' => null]);
        } else {
    ...
    public function attach_document(Request $request, $offer_id)
    {
        ...
        if ($request->commercial_offer) {
            ...
            $file_name = 'project-'.$offer->project_id.'_commercial_offer-'
                .uniqid().'.'.$mime;

            Storage::disk('commercial_offers')
                ->put($file_name, File::get($request->commercial_offer));

            FileEntry::create([
                'filename'          => $file_name,
                'size'              => $request->commercial_offer->getSize(),
        ...
        if ($request->budget) {
            ...
            $file_name = 'project-'.$offer->project_id.'_budget-'.uniqid().'.'
                .$mime;

            Storage::disk('budget')
                ->put($file_name, File::get($request->budget));

            FileEntry::create([
                'filename'          => $file_name,
                'size'              => $request->budget->getSize(),
                'mime'              => $request->budget->getClientMimeType(),
                'original_filename' => $request->budget->getClientOriginalName(),
                'user_id'           => Auth::user()->id,
            ]);
        ...
        if ($offer->is_tongue == 1) {
            foreach ([5, 6] as $group_id) {
                $prev_task = Task::where('target_id', $offer->id)
                    ->where('status', 5)->where('is_solved', 0)->first();
                $task      = new Task([
                    'project_id'          => $offer->project_id,
                    'name'                => 'Согласование КП (шпунт)',
                    'responsible_user_id' => Group::find($group_id)->getUsers()
                        ->first()->id,
                ...
                $this->prepareNotifications[\App\Notifications\CommercialOffer\CustomerApprovalOfOfferSheetPilingTaskNotice::class][]
                    ...
                    'url'             => $task->task_route(),
                    'task_id'         => $task->id,
                    'contractor_id'   => $task->project_id
                        ? Project::find($task->project_id)->contractor_id
                        : null,
                    'project_id'      => $task->project_id ? $task->project_id
                        : null,
                    'object_id'       => $task->project_id
                        ? Project::find($task->project_id)->object_id : null,
                ];
        ...
        return redirect()->route('projects::card', $offer->project_id)
            ->with('com_offer', true);
    }
    ...
    public function agree_commercial_offer($offer_id): RedirectResponse
    {
        ...
        $task = Task::where('project_id', $offer->project_id)
            ->where('status', 6)->where(function ($q) {
        ...
        if ($task) {
            $this->prepareNotifications[\App\Notifications\Task\TaskClosureNotice::class][]
                ...
                'task_id'       => $task->id,
                'contractor_id' => $task->project_id ? $project->contractor_id
                    : null,
                'project_id'    => $task->project_id ? $task->project_id : null,
                'object_id'     => $task->project_id ? $project->object_id
                    : null,
            ];
            ...
            $this->prepareNotifications[\App\Notifications\Task\TaskClosureNotice::class][]
                = [
                'user_ids'        => Group::find(5/*3*/)->getUsers()
                    ->first()->id,
                'name'            => 'Задача «'.$task->name
                    .'» закрыта с результатом: '.$status_for_humans['accept'].
                    (is_null($task->revive_at)
                        ? ''
                        : '. Дата, на которую перенесли: '.
                        strftime('%d.%m.%Y', strtotime($task->revive_at))).
                    (is_null($task->final_note) ? ''
                        : '. Комментарий: '.$task->final_note),
                'additional_info' => "\r\nЗаказчик: ".$project->contractor_name.
                    "\r\nНазвание объекта: ".$project->object->name.
                    "\r\nАдрес объекта: ".$project->object->address.
                    "\r\nИсполнитель: "
                    .User::find($task->responsible_user_id)->long_full_name,
                'url'             => route('projects::card',
                    [$task->project_id, 'task' => $task->id]),
                'task_id'         => $task->id,
                'status'          => 2,
                'contractor_id'   => $task->project_id ? $project->contractor_id
                    : null,
        ...
        if ($project->respUsers()->where('role', ($offer->is_tongue ? 6 : 5))
                ->count() > 0
        ) {
            ...
            if ($offer->is_tongue) {
                $mainEngineer   = Group::find($mainEngineerOfTonguePostId)
                    ->getUsers()->first();
                $taskNameSuffix = '(шпунт)';
                $taskStatus     = 25;
            } else {
                $mainEngineer   = Group::find($mainEngineerOfPilesPostId)
                    ->getUsers()->first();
                $taskNameSuffix = '(сваи)';
                $taskStatus     = 24;
            }
            ...
            $add_RP_task = Task::create([
                'project_id'          => $project->id,
                'name'                => 'Назначение ответственного руководителя проектов '
                    .$taskNameSuffix,
                'responsible_user_id' => $mainEngineer ? $mainEngineer->user_id
                    : 6,
                'contractor_id'       => $project->contractor_id,
                'target_id'           => $offer->id,
            ...
            $this->prepareNotifications[\App\Notifications\Task\ProjectLeaderAppointmentTaskNotice::class][]
                ...
                'contractor_id'   => $add_RP_task->project_id
                    ? Project::find($add_RP_task->project_id)->contractor_id
                    : null,
                'project_id'      => $add_RP_task->project_id ?: null,
                'object_id'       => $add_RP_task->project_id
                    ? Project::find($add_RP_task->project_id)->object_id : null,
            ];
        }
        ...
        if ($project->status < 4 || $project->status == 5) {
            ...
            foreach (
                ProjectResponsibleUser::where('project_id', $project->id)
                    ->where('role', 7)->get() as $user
            ) {
                $task = new Task([
                    'project_id'          => $offer->project_id,
    ...
    public function create_double_kp(Request $request)
    {
        ...
        $wvPile      = WorkVolume::where('project_id', $project->id)
            ->where('status', 2)->where('type', 1)->first();
        $wvTongue    = WorkVolume::where('project_id', $project->id)
            ->where('status', 2)->where('type', 0)->first()
            ->load('shown_materials.parts');
        $doubleCO    = CommercialOffer::where('project_id', $project->id)
            ->where('status', 5)->where('is_tongue', 2)->first();
        ...
        if ( ! $doubleCO) {
            ...
            $worksPile       = WorkVolumeWork::where('work_volume_id',
                $wvPile->id)->get();
            $worksTongue     = WorkVolumeWork::where('work_volume_id',
                $wvTongue->id)->get();
            $works           = collect();
            $works           = $works->merge($worksPile)->merge($worksTongue);
            $works_materials = WorkVolumeWorkMaterial::whereIn('wv_work_id',
                $works->pluck('id'))->get();
            $materialsPile   = WorkVolumeMaterial::where('work_volume_id',
                $wvPile->id)->get();
            $materialsTongue = $wvTongue->shown_materials;
            $materials       = collect();
            $materials       = $materials->merge($materialsPile)
                ->merge($materialsTongue);
                ...
                if ($material->material_type != 'complect') {
                    ...
                    foreach ($old_relations as $item) {
                        ...
                        if ($material_id == $material->id) {
                            $new_relation[]
                                = [$new_material->id => $item[$material_id]];
                        }
                    }
                } else {
            ...
            foreach ($result as $item) {
                WorkVolumeWorkMaterial::create([
                    'wv_work_id'     => current($item),
                    'wv_material_id' => key($item),
                ]);
            }
            // get CO versions for double CO version
            $offers_countTongue = CommercialOffer::where('project_id',
                $project->id)->where('is_tongue', 1)->update(['status' => 3]);
            $offers_countPile   = CommercialOffer::where('project_id',
                $project->id)->where('is_tongue', 0)->update(['status' => 3]);

            // close all tasks for tongue and pile CO
            foreach ([0, 1] as $is_tongue) {
                $offers_id = CommercialOffer::where('project_id', $project->id)
                    ->where('is_tongue', $is_tongue)->pluck('id')->toArray();

                $tasks = Task::where('project_id', $project->id)
                    ->where('status', 6)->whereIn('target_id', $offers_id)
                    ->get();
            ...
            $commercial_offer->version        = 'Ш'.$offers_countTongue.'С'
                .$offers_countPile;
            $commercial_offer->file_name      = 0;
            ...
            foreach ([$offerTongue, $offerPile] as $prev_com_offer) {
                ...
                //take new materials
                $new_wv_mats   = WorkVolumeMaterial::where('work_volume_id',
                    $wv_double->id)->get();
                $split_adapter = $new_wv_mats->groupBy('manual_material_id')
                    ->map(function ($group) {
                ...
                $control_count
                    = $prev_com_offer->mat_splits->groupBy('man_mat_id'); //here are old ones
                //creating splits for new commercial_offer
                foreach ($split_adapter as $manual_id => $count) {
                    if (in_array($manual_id,
                        array_keys($control_count->toArray()))
                    ) {
                        if (($count
                            == $control_count[$manual_id]->whereIn('type',
                                [1, 3, 5])->sum('count'))
                        ) { //if there was no changes amount of
                            foreach ($control_count[$manual_id] as $old_split) {
                                $new_split = $old_split->replicate();
                                $new_split->man_mat_id
                                           = $old_split->man_mat_id;
                                $new_split->com_offer_id
                                           = $commercial_offer->id;
                                $new_split->save();
                            }
                        } else {
            ...
            $com_offer_request->status              = 0;
            $com_offer_request->description
                                                    = 'Было создано объединенное КП, проверьте актуальность цен';
            ...
            $task_1 = Task::create([
                ...
                'responsible_user_id' => ProjectResponsibleUser::where('project_id',
                    $commercial_offer->project_id)->where('role', 2)
                    ->first()->user_id,
                'contractor_id'       => $project->contractor_id,
                'target_id'           => $commercial_offer->id,
                'status'              => 6,
    ...
    public function upload(CommercialOfferReqRequest $request, $project_id)
    {
        ...
        if ($request->com_offer_id_tongue != 'new') {
            $old_offer
                    = CommercialOffer::findOrFail($request->com_offer_id_tongue);
            $option = $old_offer->option;
            $old_offer->decline();
        } elseif ($request->com_offer_id_pile != 'new') {
            $old_offer
                    = CommercialOffer::findOrFail($request->com_offer_id_pile);
            $option = $old_offer->option;
            $old_offer->decline();
        }

        $existing_WVs = WorkVolume::where('project_id', $project_id)
            ->where('option', $option);
        ...
        $offer = new CommercialOffer([
            ...
            'name'           => 'Коммерческое предложение ('
                .($request->is_tongue ? 'шпунтовое' : 'свайное')
                .' направление) загружено',
            'file_name'      => '0',
            'user_id'        => Auth::id(),
            'status'         => '1',
        ...
        if ($request->negotiation_type == 1) {
            ...
            if ($project->respUsers()
                    ->where('role', ($offer->is_tongue ? 6 : 5))->count() > 0
            ) {
                $mainEngineerOfTonguePostId = 8;
                ...
                $add_RP_task = Task::create([
                    ...
                    'name'                => 'Назначение ответственного руководителя проектов '
                        .$taskNameSuffix,
                    'responsible_user_id' => $mainEngineer
                        ? $mainEngineer->user_id : 6,
                    'contractor_id'       => $project->contractor_id,
                    'target_id'           => $offer->id,
                    'status'              => $taskStatus,
                    'expired_at'          => $this->addHours(11),
                ...
                $this->prepareNotifications[\App\Notifications\Task\ProjectLeaderAppointmentTaskNotice::class][]
                    ...
                    'contractor_id'   => $add_RP_task->project_id
                        ? Project::find($add_RP_task->project_id)->contractor_id
                        : null,
                    'project_id'      => $add_RP_task->project_id
                        ? $add_RP_task->project_id : null,
                    'object_id'       => $add_RP_task->project_id
                        ? Project::find($add_RP_task->project_id)->object_id
                        : null,
                ];
            }
        }
    ...
    public function upload_signed_pdf($project_id)
    {
        ...
        if (request()->hash) {
            ...
            $path_to_save
                              = storage_path('app/public/docs/commercial_offers');
            $file_name        = 'project-'.$project_id.'_commercial_offer-'
                .uniqid().'.pdf';
    ...
    public function get_review(Request $request)
    {
        ...
        if (in_array($review_type,
            ['MaterialWorkRelation', \App\Models\Manual\ManualWork::class])
        ) {
            $review->where('commercial_offer_id',
                $request->commercial_offer_id);
        }
        $review = $review->first();
        $text   = '';
        if ($review) {
    ...
    public function store_review(Request $request)
    {
        if (strpos($request->form_reviewable_type, '.') === false) {
            ...
            $review_type = str_replace('.', '\\',
                $request->form_reviewable_type);
        }
    ...
    {
        foreach ($this->prepareNotifications as $class => $notifications) {
            foreach ($notifications as $arguments) {
                try {
                    ...
                    $message
                                    = "В контроллере $controllerName, не удалось отправить уведомление $class, возникла ошибка: ";
