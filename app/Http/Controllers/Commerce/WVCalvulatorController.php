...
class WVCalvulatorController extends Controller
{
    ...
    public function create_tongue_calc(Request $request, $work_volume_id)
    {
        ...
        $work_volume_works = WorkVolumeWork::where('work_volume_id',
            $work_volume_id)->pluck('manual_work_id')->toArray();
        $worksWithCount    = (new ManualWork())->worksWithCount();

        $materials = ManualMaterial::whereIn('id',
            [$request->tongue_type, $request->type_angle])
            ->with('parameters')
            ->get();
        ...
        if ($request->project_count >= $request->tongue_count) {
            ...
            $tongue_count = isset($request->is_required_count)
                ? $request->project_count : $request->tongue_count;
        }

        $length_tongue = $materials->where('id', $request->tongue_type)
            ->first()->parameters->where('unit', 'м')->first()->value ?? 0;

        if ($request->type_angle) {
            $length_angle = $materials->where('id', $request->type_angle)
                ->first()->parameters->where('unit', 'м.п')->first()->value;

            $new_angle_unit_value = $angle_material->convert_to('т');
            if ($new_angle_unit_value) {
                $result_angle_count = $new_angle_unit_value->value
                    * $request->count_angle;
            } else {
                return back();
            }
        ...
        if ($new_tongue_unit_value) {
            $result_tongue_count = $new_tongue_unit_value->value
                * $result_tongue_count;
        } else {
            return back();
        }
        ...
        $new_work_tongue     = WorkVolumeMaterial::create([
            ...
            'is_tongue'          => 1,
            'price_per_one'      => $materials->where('id',
                $request->tongue_type)->first()->buy_cost,
            'result_price'       => $result_tongue_count
                * $materials->where('id', $request->tongue_type)
                    ->first()->buy_cost,
            'unit'               => 'т',
        ]);
        ...
        if ($request->count_angle and $request->type_angle) {
            $new_work_angle = WorkVolumeMaterial::create([
                ...
                'is_tongue'          => 1,
                'price_per_one'      => $materials->where('id',
                    $request->type_angle)->first()->buy_cost,
                'result_price'       => $request->count_angle
                    * $materials->where('id', $request->type_angle)
                        ->first()->buy_cost,
                'unit'               => 'т',
            ]);
        }
        ...
        $manual_material
            = WorkVolumeMaterial::whereIn('work_volume_materials.id',
            [$new_work_tongue->id])
            ->with('manual.parameters')
            ->leftJoin('manual_materials', 'manual_materials.id', '=',
                'work_volume_materials.manual_material_id')
            ->leftJoin('manual_material_categories',
                'manual_material_categories.id', '=',
                'manual_materials.category_id')
            ->select('work_volume_materials.*',
                'manual_material_categories.name',
                'manual_material_categories.category_unit',
                'manual_materials.category_id')
            ->get();

        $manual_works = ManualWork::whereIn('id',
            [4, 6, 9, 10, 15, 16, 19, 20, 27, 28])->get();
        ...
        foreach ($manual_material as $material) {
            foreach ($manual_works as $work) {
                if ($material->category_unit == $work->unit) {
                    $count_work_1[$work->id.$work->name]
                        += round($material->count
                        / $new_tongue_unit_value->value, 3);
                }
            }
        }
            ...
            foreach ($material->parameters as $parameter) {
                foreach ($manual_works as $work) {
                    if ($parameter->unit == $work->unit) {
                        $count_work_2[$work->id.$work->name]
                            += round($material->count, 3);
                    }
                }
            }
        }
        ...
        $work_counts_tongue = array_values(array_merge($count_work_2,
            $count_work_1));
        ...
        if ($request->count_angle and $request->type_angle) {
            ...
            $work_counts_angle = array_values(array_merge($count_work_2,
                $count_work_1));
        }
        ...
        foreach ($manual_works as $key => $work) {
            ...
            if ($work->id == 9 or $work->id == 19 or ($work->id == 20
                    and $request->is_out) or ($work->id == 10
                    and $request->is_out)
            ) {
                if ($request->count_angle and $request->type_angle) {
                    ...
                    $new_works[] = [
                        ...
                        'result_price'   => $work_counts_angle[$key]
                            * $work->price_per_unit,
            ...
            if ($work->id == 4 or $work->id == 6 or $work->id == 9 or $work->id
                == 10 or $work->id == 16 or ($work->id == 20
                    and $request->is_out) or $work->id == 15 or $work->id == 19
                or $work->id == 28 or $work->id == 27
            ) {
                ...
                $new_works[] = [
                    ...
                    'is_tongue'      => 1,
                    'price_per_one'  => $work->price_per_unit,
                    'result_price'   => $work_counts_tongue[$key]
                        * $work->price_per_unit,
                    'created_at'     => Carbon::now(),
                    'updated_at'     => Carbon::now(),
                ];
        ...
        foreach ($new_works as $item) {
            $find_work = WorkVolumeWork::where('work_volume_id',
                $work_volume_id)
            ...
            if ($item['manual_work_id'] == 4 or $item['manual_work_id'] == 6) {
                ...
                $new_work->count += in_array($item['manual_work_id'],
                    $worksWithCount) ? round($item['count'], 0)
                    : $item['count'];
                $new_work->term  += ceil($item['term']);
            }
            $new_work->is_tongue = 1;
            $new_work->order     = WorkVolumeWork::where('work_volume_id',
                    $work_volume_id)->max('order') + 1;

            // TODO make nice query
            $new_work->price_per_one = $item['price_per_one'];
            $new_work->result_price  = $new_work->price_per_one
                * $new_work->count;
            ...
            if (in_array($new_work->manual_work_id, $used_works)) {
                if ($request->count_angle and $request->type_angle) {
                    if ($item['manual_work_id'] == 9 or $item['manual_work_id']
                        == 19 or ($item['manual_work_id'] == 20
                            and $request->is_out) or ($item['manual_work_id']
                            == 10 and $request->is_out)
                    ) {
                ...
                if ($item['manual_work_id'] == 4 or $item['manual_work_id']
                    == 6
                ) {
                } else {
                    $new_material = new WorkVolumeWorkMaterial();
            ...
            foreach (
                array_keys($used_works, $new_work->manual_work_id, true) as $key
            ) {
    ...
    {
        $wv_materials = ManualMaterial::where('manual_materials.category_id',
            2);

        if ($request->q) {
            $wv_materials = $wv_materials->where('manual_materials.name',
                'like', '%'.trim($request->q).'%');
        }
    ...
    {
        $parameters = ManualMaterialParameter::where('mat_id',
            $request->material_id)
            ->where('manual_material_category_attributes.name',
                'Ширина профиля по центрам замков b')
            ->leftJoin('manual_material_category_attributes',
                'manual_material_category_attributes.id',
                'manual_material_parameters.attr_id')
            ->select('manual_material_parameters.*',
                'manual_material_category_attributes.name')
            ->first();
    ...
    {
        $wv_materials = ManualMaterial::whereIn('manual_materials.category_id',
            [7, 8, 9]);
    ...
    public function calculate_mount($material, $type, $request, $work_volume_id)
    {
        ...
        $manual_works = ManualWork::whereIn('id', [43, 44, 47, 48, 49, 50])
            ->get();
        ...
        foreach ($new_works as $item) {
            ...
            $new_work->work_volume_id = $work_volume_id;
            $new_work->manual_work_id = $item['manual_work_id'];
            if ($item['manual_work_id'] == 43 or $item['manual_work_id']
                == 44
            ) {
                $new_work->count = 1;
                $new_work->term  = 1;
                ...
                if ($item['manual_work_id'] == 49) {
                    if ($type == 1) {
                        if ($mat_count == 0) {
                            $new_work->term += ceil(array_sum($request->strapping_beam_count)
                                / 8);
                        } else {
                            $new_work->term = 0;
                            $mat_count      += 1;
                        ...
                        foreach (
                            $request->corner_strut_length as $key => $value
                        ) {
                            if ($value < 12) {
                    ...
                    if ($type == 1) {
                        ...
                        if ($mat_count == 0) {
                            $new_work->term += ceil(array_sum($request->corner_strut_count)
                                / 3);
                        } else {
                            $new_work->term = 0;
                            $mat_count      += 1;
            ...
            if ($item['manual_work_id'] == 47 or $item['manual_work_id'] == 48
                or $item['manual_work_id'] == 49 or $item['manual_work_id']
                == 50
            ) {
    ...
    public function create_mount_calc(
        Request $request,
        $work_volume_id
    ): RedirectResponse {
        DB::beginTransaction();
        ...
        $materials = ManualMaterial::whereIn('id',
            array_merge($request->strapping_beam, $request->corner_strut,
                $request->cross_strut, $request->strut, $request->racks))
            ->with('parameters')
            ->get();
        ...
        if ($request->strapping_beam[0] != null) {
            foreach ($request->strapping_beam as $key => $item) {
                if ($item) {
                    $common_material  = WorkVolumeMaterial::create([
                        ...
                        'is_our'             => 1,
                        'count'              => round($request->strapping_beam_count_weight[$key],
                            3),
                        'is_tongue'          => 1,
                        'price_per_one'      => $materials->where('id', $item)
                            ->first()->buy_cost,
                        'result_price'       => $request->strapping_beam_count[$key]
                            * $materials->where('id', $item)->first()->buy_cost,
                        'created_at'         => Carbon::now(),
                        'updated_at'         => Carbon::now(),
                    ]);
            ...
            $this->calculate_mount($strapping_beam, 1, $request,
                $work_volume_id);
        }
            ...
            foreach ($request->corner_strut as $key => $item) {
                if ($item) {
                    $common_material = WorkVolumeMaterial::create([
                        ...
                        'manual_material_id' => $item,
                        'is_our'             => 1,
                        'count'              => round($request->corner_strut_count_weight[$key],
                            3),
                        'is_tongue'          => 1,
                        'price_per_one'      => $materials->where('id', $item)
                            ->first()->buy_cost,
                        'result_price'       => $request->corner_strut_length[$key]
                            * $materials->where('id', $item)->first()->buy_cost,
                        'created_at'         => Carbon::now(),
                        'updated_at'         => Carbon::now(),
                    ]);
            ...
            foreach ($request->cross_strut as $key => $item) {
                if ($item) {
                    $common_material = WorkVolumeMaterial::create([
                        ...
                        'manual_material_id' => $item,
                        'is_our'             => 1,
                        'count'              => round($request->cross_strut_count_weight[$key],
                            3),
                        'is_tongue'          => 1,
                        'price_per_one'      => $materials->where('id', $item)
                            ->first()->buy_cost,
                        'result_price'       => $request->cross_strut_count[$key]
                            * $materials->where('id', $item)->first()->buy_cost,
                        'created_at'         => Carbon::now(),
                        'updated_at'         => Carbon::now(),
                    ]);
            ...
            foreach ($request->strut as $key => $item) {
                if ($item) {
                    $common_material = WorkVolumeMaterial::create([
                        ...
                        'manual_material_id' => $item,
                        'is_our'             => 1,
                        'count'              => round($request->strut_count_weight[$key],
                            3),
                        'is_tongue'          => 1,
                        'price_per_one'      => $materials->where('id', $item)
                            ->first()->buy_cost,
                        'result_price'       => $request->strut_count[$key]
                            * $materials->where('id', $item)->first()->buy_cost,
                        'created_at'         => Carbon::now(),
                        'updated_at'         => Carbon::now(),
                    ]);
        ...
        if ($request->racks[0] != null and $request->racks_count[0] != null
            and $request->racks_length[0] != null
        ) {
            foreach ($request->racks as $key => $item) {
                if ($item) {
                    $common_material = WorkVolumeMaterial::create([
                        ...
                        'manual_material_id' => $item,
                        'is_our'             => 1,
                        'count'              => round($request->racks_count_weight[$key],
                            3),
                        'is_tongue'          => 1,
                        'price_per_one'      => $materials->where('id', $item)
                            ->first()->buy_cost,
                        'result_price'       => $request->racks_count[$key]
                            * $materials->where('id', $item)->first()->buy_cost,
                        'created_at'         => Carbon::now(),
                        'updated_at'         => Carbon::now(),
                    ]);
            ...
            foreach ($manual_material as $material) {
                foreach ($material->parameters as $parameter) {
                    foreach ($manual_works as $work) {
                        if ($parameter->unit == $work->unit) {
                            $count_work_2[$work->id.$work->name]
                                += $material->count * $parameter->value;
                        }
                    }
                }
            }
            ...
            $racks_count = array_values(array_merge($count_work_2,
                $count_work_1));
            ...
            foreach ($manual_works as $key => $work) {
                ...
                $new_works_racks[] = [
                    ...
                    'term'           => ceil($racks_count[$key]
                        / $work->unit_per_days),
                    'is_tongue'      => 1,
                    'price_per_one'  => $work->price_per_unit,
                    'result_price'   => $racks_count[$key]
                        * $work->price_per_unit,
                    'created_at'     => Carbon::now(),
                    'updated_at'     => Carbon::now(),
                ];
            ...
            foreach ($new_works_racks as $key_req => $item) {
                ...
                $new_work->manual_work_id = $item['manual_work_id'];
                $new_work->count          += $item['count'];
                if ($item['manual_work_id'] == 51 or $item['manual_work_id']
                    == 52
                ) {
                    if ($racks_mat_count == 0) {
                        $new_work->term += ceil(array_sum($request->racks_count)
                            / 8);
                    } else {
                        $new_work->term  = 0;
                        $racks_mat_count += 1;
        ...
        $material_id_embedds = [];
        if ($request->embedded_parts[0] != null
            and $request->embedded_parts_count[0] != null
        ) {
            foreach ($request->embedded_parts as $key_main => $item) {
                $manual_materials_id    = ManualNodeMaterials::where('node_id',
                    $item)->pluck('manual_material_id')->toArray();
                $manual_materials_value = ManualNodeMaterials::where('node_id',
                    $item)->pluck('count')->toArray();
                ...
                foreach ($manual_materials_id as $key => $id) {
                    $emb_id = WorkVolumeMaterial::create([
                        ...
                        'is_our'             => 1,
                        'count'              => round($manual_materials_value[$key]
                                * $request->embedded_parts_count[$key_main], 2)
                            * 1.05,
                        'is_tongue'          => 1,
                        'price_per_one'      => isset($materials->where('id',
                                $id)->first()->buy_cost)
                            ? $materials->where('id', $id)->first()->buy_cost
                            : 0,
                        'result_price'       => $manual_materials_value[$key]
                        * $request->embedded_parts_count[$key_main]
                        * isset($materials->where('id', $id)->first()->buy_cost)
                            ? $materials->where('id', $id)->first()->buy_cost
                            : 0,
                        'created_at'         => Carbon::now(),
                        'updated_at'         => Carbon::now(),
            ...
            $manual_material
                ...
                ->select('work_volume_materials.*',
                    ...
                    'manual_material_categories.category_unit',
                    'manual_materials.category_id',
                    'manual_materials.id as manual_id')
                ->get();

            $manual_works = ManualWork::whereIn('id', [43, 44, 47, 48, 50, 66])
                ->get();
            ...
            foreach ($manual_material as $material) {
                foreach ($manual_works as $work) {
                    if ($material->category_unit == $work->unit) {
                        $count_work_1[$work->id.$work->name]
                                            += (float) number_format($material->count,
                            3);
            ...
            $embedded_parts_count = array_values(array_merge($count_work_2,
                $count_work_1));
            ...
            foreach ($manual_works as $key => $work) {
                $new_works_embedded_parts[] = [
                    ...
                    'term'           => ceil($embedded_parts_count[$key]
                        / $work->unit_per_days),
                    'is_tongue'      => 1,
                    'price_per_one'  => $work->price_per_unit,
                    'result_price'   => $embedded_parts_count[$key]
                        * $work->price_per_unit,
                    'created_at'     => Carbon::now(),
                    'updated_at'     => Carbon::now(),
                ];
            ...
            foreach ($new_works_embedded_parts as $item) {
                ...
                if ($item['manual_work_id'] == 43 or $item['manual_work_id']
                    ...
                    $new_work->count = 1;
                    $new_work->term  = 1;
                } elseif ($item['manual_work_id'] == 66
                    or $item['manual_work_id'] == 50
                ) {
                    $new_work->count += $item['count'];
                    $new_work->term  += 0;
                } else {
        ...
        if ((($request->nodes_count[0] ?? null) and ($request->nodes[0] ??
                    null)) or (($request->sheets_count[0] ?? null)
                and ($request->sheets[0] ?? null))
        ) {
            if ((($request->nodes_count[0] ?? null) and ($request->nodes[0] ??
                    null))
            ) {
                foreach ($request->nodes as $key_main => $item) {
                    if ($item) {
                        $manual_materials_id
                                   = ManualNodeMaterials::where('node_id',
                            $item)->pluck('manual_material_id')->toArray();
                        $manual_materials_value
                                   = ManualNodeMaterials::where('node_id',
                            $item)->pluck('count')->toArray();
                        $node
                                   = ManualNodes::with('node_materials')
                            ->findOrFail($item);
                        $materials = ManualMaterial::whereIn('id',
                            $manual_materials_id)
                            ->with('parameters')
                        ...
                        foreach ($manual_materials_id as $key => $id) {
                            $node_mat            = WorkVolumeMaterial::create([
                                ...
                                'is_our'             => 1,
                                'count'              => round($manual_materials_value[$key]
                                        * $request->nodes_count[$key_main], 3)
                                    * (1 + $node->node_category->safety_factor
                                        / 100),
                                'is_tongue'          => 1,
                                'price_per_one'      => isset($materials->where('id',
                                        $id)->first()->buy_cost)
                                    ? $materials->where('id', $id)
                                        ->first()->buy_cost : 0,
                                'result_price'       => $manual_materials_value[$key]
                                * $request->nodes_count[$key_main]
                                * isset($materials->where('id', $id)
                                        ->first()->buy_cost)
                                    ? $materials->where('id', $id)
                                        ->first()->buy_cost : 0,
                                'created_at'         => Carbon::now(),
            ...
            if (($request->sheets[0] ?? null) and ($request->sheets_count[0] ??
                    null)
            ) {
                ...
                foreach ($request->sheets as $key => $id) {
                    if ($id) {
                        $sheets_mat          = WorkVolumeMaterial::create([
                            ...
                            'manual_material_id' => $id,
                            'is_our'             => 1,
                            'count'              => round($request->sheets_count[$key],
                                3),
                            'is_tongue'          => 1,
                            'price_per_one'      => isset($materials->where('id',
                                    $id)->first()->buy_cost)
                                ? $materials->where('id', $id)
                                    ->first()->buy_cost : 0,
                            'result_price'       => $request->sheets_count[$key]
                            * isset($materials->where('id', $id)
                                    ->first()->buy_cost)
                                ? $materials->where('id', $id)
                                    ->first()->buy_cost : 0,
            ...
            $nodes_count = array_values(array_merge($count_work_2,
                $count_work_1));
            foreach ($manual_works as $key => $work) {
                if ($work->id == 48 or $work->id == 50) {
                    if ( ! $request->is_out) {
                ...
                $new_work_nodes[] = [
                    ...
                    'term'           => ceil(($nodes_count[$key])
                        / $work->unit_per_days),
                    'is_tongue'      => 1,
                    'price_per_one'  => $work->price_per_unit,
                    'result_price'   => $nodes_count[$key]
                        * $work->price_per_unit,
                    'created_at'     => Carbon::now(),
                    'updated_at'     => Carbon::now(),
                ];
    ...
    public function count_weight(Request $request): JsonResponse
    {
        ...
        if ($request->mat_id) {
            $material_value = ManualMaterial::where('id', $request->mat_id)
                ->first()->parameters()->orderBy('id', 'desc')
                ->where('unit', 'кг')->first()->value ?? 0;
        }

        return response()->json([
            'material_value' => ($material_value / 1000) ?? 0,
        ]);
    }
