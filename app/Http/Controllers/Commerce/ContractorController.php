...
class ContractorController extends Controller
{
    ...
    public function store(ContractorStoreRequest $request): RedirectResponse
    {
        ...
        foreach ($request->phone_count as $phone => $main_id) {
            if (trim(preg_replace('~[\D]~', '', $request->phone_number[$phone]))
                ...
                ContractorPhone::create([
                    'name'          => isset($request->phone_name[$phone])
                        ? $request->phone_name[$phone] : '',
        ...
        if ($request->task_id) {
            return redirect()->route('tasks::new_call',
                [$request->task_id, 'contractor_id' => $contractor->id]);
        }
    ...
    public function search_dadata(Request $request)
    {
        ...
        try {
            $result = DadataSuggest::suggest($type,
        ...
        if ($request->is_edit) {
            return view('contractors.edit', [
                'type'       => $type,
                'result'     => ! empty($result['suggestions'])
                    ? ($result['suggestions'][isset($request->id) ? $request->id
                        : 0]) : '',
                'results'    => $result['suggestions'],
                'old_search' => $request->search ? $request->search : '',
                'contractor' => Contractor::findOrFail($request->contractor_id),
                'bank'       => BankDetail::where('contractor_id',
                    $request->contractor_id)->first(),
            ]);
        }
    ...
    {
        $projects   = Project::getAllProjects()->with([
            'contracts' => function ($q) {
                $q->withCount('get_requests');
            },
        ])->where('projects.contractor_id', $id);
        $com_offers = CommercialOffer::whereIn('project_id',
            $projects->pluck('id'))
        ...
        foreach ($solved_tasks as $solved_task) {
            $com_for_tasks = CommercialOffer::where('project_id',
                $solved_task->project_id)
                ->whereIn('commercial_offers.status', [1, 2, 4, 5])->first();
            $wv_for_tasks  = WorkVolume::where('work_volumes.project_id',
                $solved_task->project_id)
                ->whereIn('work_volumes.status', [1, 2])->first();

            $solved_task->commercial_offer_id   = is_null($com_for_tasks) ? ''
                : $com_for_tasks->id;
            $solved_task->work_volume_id        = is_null($wv_for_tasks) ? ''
                : $wv_for_tasks->id;
        ...
        $projects_com_offer = Project::whereIn('id',
            $contracts->pluck('project_id')->toArray())->get();

        return view('contractors.card', [
            'contractor'             => $contractor,
            'bank'                   => BankDetail::where('contractor_id', $id)
                ->first(),
            'contacts'               => $contacts,
    ...
    public function add_contact(
        ...
        if ($request->task_id) {
            return redirect()->route('tasks::new_call', [
                $request->task_id, 'contractor_id' => $id,
    ...
    public function edit_contact(ContractorContactRequest $request
        ...
        if ($request->project_note) {
            $p_contact       = ProjectContact::where('contact_id', $contact->id)
    ...
    public function is_unique(Request $request)
    {
        if ($request->full_name) {
            if (Contractor::where('full_name', $request->full_name)
        ...
        if ($request->inn) {
            if (Contractor::where('inn', $request->inn)
                ->where('id', '!=', $request->id)->first()
            ) {
        ...
        if ($request->ogrn) {
            if (Contractor::where('ogrn', $request->ogrn)
                ->where('id', '!=', $request->id)->first()
            ) {
        ...
        if ($request->email) {
            if (Contractor::where('email', $request->email)
                ->where('id', '!=', $request->id)->first()
            ) {
    ...
    public function contractor_delete_request(Request $request)
    {
        ...
        if (Task::where('status', 19)
            ->where('target_id', $request->contractor_id)->where('is_solved', 0)
            ->first()
        ) {
            return back();
        }
        ...
        // create task
        $task = Task::create([
            'name'                => 'Контроль удаления контрагента '
                .$request->contractor_name,
            'description'         => 'Пользователь '.Auth::user()->full_name.
                ' отправил заявку на удаление контрагента с комментарием: '
                .$request->reason.
    ...
    public function solve_remove(Request $request, $id)
    {
        ...
        $contractor = Contractor::findOrFail($task->contractor_id)
            ->load('projects');
        ...
        if ($request->status_result == 'accept') {
            if (Schema::hasColumn($contractor->getTable(), 'deleted_at')) {
                ...
                return back()->with('no_soft',
                    'Возникли проблемы с удалением, пожалуйста, свяжитесь с разработчиками');
            }
        }
        ...
        ContractorDeletionControlTaskResolutionNotice::send(
            $task->user_id,
            [
                'name'          => 'Запрашиваемый вами контрагент '
                    .$contractor->short_name.' '
                    .$task->results[$task->status][$task->result].
                    ($request->description ? ', с комментарием: '
                        .$request->description : ''),
                'task_id'       => $task->id,
                'contractor_id' => $task->contractor_id,
            ]
    ...
    public function get_by_type()
    {
        ...
        foreach ($contractors as $contractor) {
            $results[] = [
                'id' => $contractor->id.'', 'text' => $contractor->short_name,
            ];
        }
    ...
    public function getContractors(Request $request)
    {
        ...
        return $contractors->limit(20)->get()->map(function ($contractor) {
            return [
                'code' => $contractor->id.'', 'label' => $contractor->short_name,
            ];
        });
    }

    public function solveTaskCheckContractor(
        Request $request,
        $task_id
    ): RedirectResponse {
        ...
        $solvedTasks = Task::whereStatus($task->status)
            ->whereContractorId($contractor->id)->get();
