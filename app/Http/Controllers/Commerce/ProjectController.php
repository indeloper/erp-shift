...
class ProjectController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        //            ->with('last_task', 'last_task.redirects', 'last_task.task_files',
        //                'last_task.responsible_user',
        ...
        //            $result = array_filter($projects->getModel()->project_status,
        //                function ($item) use ($search) {
        //                    return stristr(mb_strtolower($item), $search);
        //                });
        //
        //            $entity = array_filter($projects->getModel()::$entities,
        //                function ($item) use ($search) {
        //                    return stristr(mb_strtolower($item), $search);
        //                });
        //
        //            $projects = $projects->where(function ($q) use (
        //                $request,
        //                $result,
        //                $entity
        //            ) {
        //                $q->where('projects.name', 'like', '%'.$request->search.'%')
        //                    ->orWhere('project_objects.address', 'like',
        //                        '%'.$request->search.'%')
        //                    ->orWhere('project_objects.short_name', 'like',
        //                        '%'.$request->search.'%')
        //                    ->orWhere(DB::raw('CONCAT(users.last_name, " ", users.first_name, " ", users.patronymic)'),
        //                        'like', '%'.$request->search.'%')
        //                    ->orWhere(DB::raw('CONCAT(contractors.short_name, " ", contractors.inn)'),
        //                        'like', '%'.$request->search.'%')
    ...
    public function store(ProjectRequest $request)
    {
        ...
        $project->contractor_id
            = $request->contractor_ids[array_search($request->main_contractor,
            $request->contractor_ids)];
        ...
        $other_contractors = array_diff($request->contractor_ids,
        ...
        if ($request->has('contractor_contact_ids')) {
            return $this->updateContacts($request->contractor_contact_ids,
                $request->project_contact_ids, $project->contractor_id,
                $project->id);
        }
    ...
    public function users(Project $project): View
    {
        ...
        return view('human_resources.users_wrapper', [
            'data' => [
                ...
                'project' => $project,
                'project_users' => array_reverse($project->allUsers()
                    ->toArray()),
                'source' => 'project',
            ],
        ]);
    ...
    public function card(Request $request, $id): View
    {
        ...
        $creater = User::getAllUsers()->where('users.id', $project->user_id)
            ->first();

        $p_contacts = ProjectContact::where('project_id', $id)
            ->pluck('contact_id');
        $p_users = ProjectResponsibleUser::where('project_id', $id)
            ->pluck('user_id')->toArray();

        $contacts = ContractorContact::with('phones')
            ->whereIn('contractor_contacts.id', $p_contacts)
            ...
            ->select('contractor_contacts.*',
                ...
                'project_contacts.note as proj_contact_note')
            ->get();
        foreach ($contacts as $contact) {
            foreach ($contact->phones as $phone) {
                preg_match("/^(\d{1})(\d{3})(\d{3})(\d{2})(\d{0,2})$/",
        ...
        $resp_users = User::getAllUsers()->whereIn('users.id', $p_users)
            ...
            ->leftJoin('project_responsible_users',
                function ($leftJoin) use ($id) {
                    $leftJoin->on('users.id', '=',
                        'project_responsible_users.user_id');
                    $leftJoin->on(DB::raw('project_responsible_users.project_id'),
                        DB::raw('='), DB::raw("'".$id."'"));
                })
            ->select('users.id', 'users.last_name', 'users.first_name',
                'users.patronymic', 'users.birthday',
                'users.email', 'users.person_phone', 'users.work_phone',
                'users.status', 'departments.name as dep_name',
                'groups.name as group_name',
                'project_responsible_users.id as resp_user_id',
                'project_responsible_users.role as role',
                'project_responsible_users.user_id as user_id')
            ->get();

        $project_docs = ProjectDocument::where('project_id', $id)
            ->leftjoin('users', 'users.id', '=', 'project_documents.user_id')
            ->select('project_documents.*', 'users.last_name',
        ...
        $extra_documents = ExtraDocument::orderBy('version', 'desc')
            ...
            ->select('extra_documents.*', 'users.last_name', 'users.first_name',
                'users.patronymic')
            ->get();
        //
        ...
        $task_files = TaskFile::whereIn('task_files.task_id',
            $solved_tasks->pluck('id'))
            ->leftJoin('users', 'users.id', '=', 'task_files.user_id')
            ->select('users.last_name', 'users.first_name', 'users.patronymic',
                'task_files.*');

        $task_redirects = TaskRedirect::select('task_redirects.*',
            'tasks.project_id')
            ->leftJoin('tasks', 'tasks.id', 'task_redirects.task_id')
            ->where('tasks.project_id', $id);

        $task_responsible_users = User::select('users.id', 'users.first_name',
            'users.last_name', 'users.patronymic')
            ->whereIn('users.id', $solved_tasks->pluck('responsible_user_id'))
            ->orWhereIn('users.id',
                $task_redirects->take(6)->pluck('old_user_id'))
            ->orWhereIn('users.id',
                $task_redirects->take(6)->pluck('responsible_user_id'))
            ->orWhereIn('users.id', $solved_tasks->pluck('description'));

        $com_offers_task = CommercialOffer::whereIn('project_id',
            $solved_tasks->pluck('project_id'));
        ...
        $work_volume_requests = WorkVolumeRequest::where('project_id', $id)
            ...
            ->select('work_volume_requests.*', 'users.last_name',
                'users.first_name', 'users.patronymic')
            ->with('files');
        ...
        foreach ($solved_tasks as $solved_task) {
            ...
            $solved_task->commercial_offer_id = is_null($com_for_tasks) ? ''
                : $com_for_tasks->id;
            //$solved_task->work_volume_id = is_null($wv_for_tasks)?'':$wv_for_tasks->id;
            $solved_task->commercial_offer_file = is_null($com_for_tasks) ? ''
                : $com_for_tasks->file_name;
        }
        ...
        $COpile = ProjectResponsibleUser::where('project_id', $id)
            ->where('role', 1)->first();
        $COtongue = ProjectResponsibleUser::where('project_id', $id)
            ->where('role', 2)->first();

        $contracts = Contract::where('project_id', $id)->with('key_dates')
            ->leftJoin('users', 'users.id', '=', 'contracts.user_id')
            ->select('contracts.*', 'users.last_name', 'users.first_name',
                'users.patronymic');
        ...
        $contract_requests
            = ContractRequest::whereIn('contract_requests.contract_id',
            $contracts->get()->groupBy('id')->keys())
            ->leftJoin('contracts', 'contracts.id', '=',
                'contract_requests.contract_id')
            ->leftJoin('users', 'users.id', '=', 'contract_requests.user_id')
            ->select('contract_requests.*', 'users.last_name',
                'users.first_name', 'users.patronymic',
                'contracts.name as contract_name',
                'contracts.contract_id as contract_number')
            ->with('files');

        $contract_resp_user_ids = isset($resp_users->where('role', '7')
                ->first()->id) ? $resp_users->where('role', '7')->pluck('id')
            ->toArray() : [0];

        $respTongueKP = isset(ProjectResponsibleUser::where('project_id', $id)
                ->where('role', 2)->first()->user_id)
            ? ProjectResponsibleUser::where('project_id', $id)->where('role', 2)
                ->first()->user_id : 0;
        $respPileKP = isset(ProjectResponsibleUser::where('project_id', $id)
                ->where('role', 1)->first()->user_id)
            ? ProjectResponsibleUser::where('project_id', $id)->where('role', 1)
                ->first()->user_id : 0;

        $alwaysResp = [];
        $resp_groups = Group::whereIn('id',
            [5/*3*/, 8/*5*/, 50/*7*/, 53/*16*/, 6/*24*/, 54/*35*/])->get();
        foreach ($resp_groups as $group) {
            $alwaysResp = array_merge($alwaysResp,
                $group->getUsers()->pluck('id')->toArray());
        }
        $all_resp_users = array_unique(array_merge($resp_users->pluck('user_id')
            ->toArray(), $alwaysResp));
        $contractors = Contractor::where('in_archive', 0)->get();

        // for new agreeKP logic
        $agree_tasks = Task::where('project_id', $id)->where('status', 6)
            ->where(function ($q) {
        ...
        $work_volumes_options = WorkVolume::where('project_id', $id)
            ...
            ->orderBy('version', 'asc')
            ->groupBy('type', 'option')
            ->select('work_volumes.*', DB::raw('max(version) as version'),
                DB::raw('max(id) as id'))
            ->get();
        ...
        $work_volumes_all = WorkVolume::where('project_id', $id)
            ->where('type', '!=', 2)
            ->orderBy('work_volumes.version', 'desc')
            ->with('get_requests');
        ...
        return view('projects.card', [
            ...
            'com_offersForeach' => $com_offers->get()
                ->sortByDesc('version')->groupBy(['is_tongue', 'option']),
            ...
            'work_volumesForeach' => $work_volumes->get()
                ->sortByDesc('version')->groupBy(['type', 'option']),
            'work_volumes' => $work_volumes->orderBy('id', 'desc')
                ->groupBy(['type', 'option'])->get(),
            ...
            'contracts' => $contracts->orderBy('contract_id',
                'desc')->orderBy('version', 'desc')->get(),
            ...
            'additional_contractors' => $project->contractors->count()
                ? $project->contractors : null,
        ]);
    }
    ...
    public function select_contacts(Request $request, $id): RedirectResponse
    {
        ...
        return redirect()->route('projects::card', $id)
            ->with('contacts', 'Новый контакт добавлен');
    }
    ...
    public function get_contacts(Request $request, $contractor_id)
    {
        ...
        $p_contacts = ProjectContact::where('project_id', $request->project_id)
            ->pluck('contact_id');

        if ($request->q) {
            $contacts = $contacts->where('last_name', 'like',
                '%'.trim($request->q).'%')
        ...
        foreach ($contacts as $contact) {
            $results[] = [
                ...
                'text' => $contact->last_name.' '.$contact->first_name.' '
                    .$contact->patronymic.', Должность: '.$contact->position,
            ];
        }
    ...
    {
        $contacts = ContractorContact::whereIn('contractor_id',
            $request->contractor_ids)
            ->whereNotIn('id', $request->contact_ids ?? []);
    ...
    public function get_objects(Request $request)
    {
        ...
        if ($request->q) {
            $objects = $objects->where('name', 'like',
                '%'.trim($request->q).'%')
                ->orWhere('address', 'like', '%'.trim($request->q).'%')
                ->orWhere('cadastral_number', 'like',
                    '%'.trim($request->q).'%');
        }
    ...
    public function get_users(Request $request)
    {
        ...
        if ($request->q) {
            ...
            $users
                = $users->where(DB::raw('CONCAT(last_name, " ", first_name, " ", patronymic)'),
                'like', '%'.$request->q.'%');
        ...
        if ($request->has('role')) {
            ...
            if ($role == 1 || $role == 2 || $role == 3) {
                ...
                $usersFromGroup = $this->findAllUsersAndReturnGroupIds([
                    53, 52, 54, 50, 74,
                ]);
                ...
                $users->whereIn('users.group_id',
                    array_unique(array_merge(['8'/*'5'*/, '27'/*'34'*/, '13'],
                        $usersFromGroup))
                );
            } elseif ($role == 7) {
                $usersFromGroup = $this->findAllUsersAndReturnGroupIds([
                ...
                $users->whereIn('users.group_id',
                    array_unique(array_merge([54/*26*/, 49/*32*/],
                        $usersFromGroup))
                );
        ...
        foreach ($users as $user) {
            $results[] = [
                ...
                'text' => $user->last_name.' '.$user->first_name.' '
                    .$user->patronymic.', Должность: '.$user->group_name,
            ];
        }
    ...
    public function get_contractors(Request $request)
    {
        ...
        if ($request->q) {
            $contractors = $contractors->where('full_name', 'like',
                '%'.trim($request->q).'%')
        ...
        $contractors = $contractors->where('in_archive', 0)
            ->whereNotIn('id',
                $request->contractor_ids ? $request->contractor_ids : [])
            ->get();
    ...
    public function select_user(SelectResponsibleUserRequest $request, $id)
    {
        ...
        $resp_user = ProjectResponsibleUser::where([
            'project_id' => $id, 'role' => $request->role,
        ]);
        $contract_resp = collect();
        if (in_array($request->role, ['7', '8', '9'])) {
            $contract_resp = ProjectResponsibleUser::where([
                'project_id' => $id, 'role' => $request->role,
            ])->get();
        }
        ...
        if ($resp_user and !in_array($request->role, ['7', '8', '9'])) {
            ...
            $resp_user->save();
        } elseif ($contract_resp->where('user_id', $request->user)->count()
            == 0
        ) {
            // if !exist, create
        ...
        if (in_array($request->role, ['6'])) {
            if ($old_user) {
                $objectResponsibleUser
                    = ObjectResponsibleUser::where('object_id', '=',
                    $project->object_id)
            ...
            if ($objectResponsibleUser) {
                $newObjectResponsibleUserExists
                    = ObjectResponsibleUser::where('object_id', '=',
                    $project->object_id)
        ...
        if ($request->task24 or $request->task25) {
            $solved_task = Task::where('status',
                $request->task24 ? 24 : 25)->where('is_solved', 0)->first();
            ...
            $name
                = "По проекту {$project->name} ({$project->object->address}) по направлению "
                .
        ...
        if (!empty($resp_user->getChanges())) {
            ...
            $tasks = Task::where([
                'responsible_user_id' => $old_user, 'is_solved' => 0,
                'project_id' => $id,
            ])->whereIn('status', $user_roles->role_tasks[$request->role])
                ->get();
            // move all tasks from styles user to new user
            foreach ($tasks as $updated_task) {
                ...
                if ($updated_task->responsible_user_id != 6) {
                    ...
                    TaskTransferNotificationToNewResponsibleNotice::send(
                        ...
                        [
                            'name' => 'Задача «'.$updated_task->name
                                .'» передана пользователю '
                                .$new_user->long_full_name,
                            'task_id' => $updated_task->id,
                            'contractor_id' => $updated_task->project_id
                                ? $project->contractor_id : null,
                            'project_id' => $updated_task->project_id
                                ? $updated_task->project_id : null,
                            'object_id' => $updated_task->project_id
            ...
            if ($request->role == 2) {
                $task = Task::where('project_id', $id)->where('status', 15)
                    ->where('is_solved', 0)->first();

                if ($task) {
                    $project = Project::find($id);
                    $work_volumes = WorkVolume::where('project_id', $id)
                        ->where('type', 0)->whereStatus(2)->get();
                    $offers_count = CommercialOffer::where('project_id', $id)
                        ->where('is_tongue', 1)->update(['status' => 3]);

                    $task->final_note = $task->descriptions[$task->status]
                        .User::find($request->user)->full_name;
                    $task->solve_n_notify();

                    foreach ($work_volumes as $work_volume) {
                        $prev_com_offer = CommercialOffer::where('project_id',
                            $project->id)->where('is_tongue', 1)
                            ->where('work_volume_id', $work_volume->id)
                        ...
                        $commercial_offer->project_id
                            = $work_volume->project_id;
                        $commercial_offer->work_volume_id = $work_volume->id;
                        $commercial_offer->option
                            = $work_volume->option;
                        ...
                        if ($prev_com_offer) {
                            ...
                            //take new materials
                            $new_wv_mats
                                = WorkVolumeMaterial::where('work_volume_id',
                                $work_volume->id)->get();
                            $split_adapter
                                = $new_wv_mats->groupBy('manual_material_id')
                                ->map(function ($group) {
                            ...
                            $control_count
                                = $prev_com_offer->mat_splits->groupBy('man_mat_id'); //here are styles ones

                            //creating splits for new commercial_offer
                            foreach ($split_adapter as $manual_id => $count) {
                                if ($count == (isset($control_count[$manual_id])
                                        ? $control_count[$manual_id]->sum('count')
                                        : -1)
                                ) { //if there was no changes amount of
                                    foreach (
                                        $control_count[$manual_id] as $old_split
                                    ) {
                            ...
                            foreach (
                                $work_volume->materials->groupBy('manual_material_id')
                                as $man_mat_id => $wv_mats
                            ) {
                                CommercialOfferMaterialSplit::create([
                        ...
                        $task = new Task([
                            ...
                            'responsible_user_id' => ProjectResponsibleUser::where('project_id',
                                $work_volume->project_id)->where('role', 2)
                                ->first()->user_id,
                            'contractor_id' => $project->contractor_id,
                            'target_id' => $commercial_offer->id,
                            'prev_task_id' => Task::where('target_id',
                                $work_volume->id)->where('status', 18)
                                ->where('is_solved', 1)->first()->id,
                        ...
                        OfferCreationSheetPilingTaskNotice::send(
                            ...
                            [
                                ...
                                'object_id' => $task->project_id
                                    ? Project::find($task->project_id)->object_id
                                    : null,
                            ]
                        );
                        ...
                        $com_offer_request->project_id
                            = $work_volume->project_id;
                        $com_offer_request->commercial_offer_id
                            = $commercial_offer->id;
                        $com_offer_request->status = 0;
                        $com_offer_request->description
                            = 'Сформирован новый объём работ, проверьте актуальность цен';
                        $com_offer_request->is_tongue
                            = $commercial_offer->is_tongue;

                        $com_offer_request->save();
                    }
                }
            } elseif ($request->role == 4) {
                $task = Task::where('project_id', $id)->where('status', 14)
                    ->where('is_solved', 0)->first();
                ...
                if ($task) {
                    ...
                    $work_volumes = WorkVolume::whereProjectId($id)
                        ->whereType(0)->whereStatus(1)->get();
                    foreach ($work_volumes as $work_volume) {
                        $tongueTask = new Task();
                        $tongueTask->project_id = $id;
                        $tongueTask->name
                            = 'Расчёт объемов (шпунтовое направление)';
                        $tongueTask->status = 3;
                        $tongueTask->responsible_user_id = $resp_user->user_id;
                        $tongueTask->contractor_id
                            = Project::find($id)->contractor_id;
    ...
    public function delete_resp_user(Request $request): JsonResponse
    {
        ...
        $user_tasks_count = Task::where('responsible_user_id',
            $request->user_id)->where('project_id', $request->project_id)
            ->where('is_solved', 0)->count();

        if (!$user_tasks_count) {
            DB::beginTransaction();

            ProjectResponsibleUser::where('user_id', $request->user_id)
        ...
        $objectResponsibleUser = ObjectResponsibleUser::where('object_id', '=',
            $project->object_id)
    ...
    public function add_contact(
        ContractorContactRequest $request,
        $id
    ): RedirectResponse {
        DB::beginTransaction();
    ...
    public function contact_delete(Request $request)
    {
        ...
        $contact = ProjectContact::where('contact_id', $request->contact_id)
            ->where('project_id', $request->project_id);
    ...
    public function tasks(Request $request, $id): View
    {
        ...
        if ($request->search) {
            $solved_tasks = $solved_tasks->where('full_name', 'like',
                '%'.$request->search.'%')
                ->orWhere('tasks.name', 'like', '%'.$request->search.'%')
                ->orWhere('contractors.short_name', 'like',
                    '%'.$request->search.'%');
        }

        $task_files = TaskFile::whereIn('task_files.task_id',
            $solved_tasks->pluck('tasks.id'))
            ->leftJoin('users', 'users.id', '=', 'task_files.user_id')
            ->select('users.last_name', 'users.first_name', 'users.patronymic',
                'task_files.*');
        ...
        $task_responsible_users = User::select('users.id', 'users.first_name',
            ...
            ->orWhereIn('users.id',
                $task_redirects->pluck('responsible_user_id'))
            ->orWhereIn('users.id', $solved_tasks->pluck('description'));

        $com_offers_task = CommercialOffer::whereIn('project_id',
            $solved_tasks->pluck('project_id'))->get();
    ...
    public function get_project_documents(Request $request, $project_id)
    {
        ...
        if ($request->q) {
            $documents = $documents->where('name', 'like',
                '%'.trim($request->q).'%');
        }
    ...
    {
        // selected person
        $contact = ContractorContact::with('phones')
            ->where('contractor_contacts.id', $contact_id)
            ->leftJoin('project_contacts',
        ...
        $html = view('projects.contact-row',
            ['contact' => $contact, 'key' => $key])->render();
    ...
    public function store_temp_contact(Request $request): JsonResponse
    {
        ...
        return response()->json([
            ...
            'html' => $this->render_contact($request->contact_id,
                $request->key),
        ]);
    }

    public function updateContacts(
        iterable $contractor_contact_ids,
        iterable $project_contacts_ids,
        int $contractor_id,
        int $project_id
    ): JsonResponse {
        DB::beginTransaction();

        ContractorContact::whereIn('id', $contractor_contact_ids)
            ->update(['contractor_id' => $contractor_id]);
        ProjectContact::whereIn('id', $project_contacts_ids)
    ...
    public function close_project(
        ...
        foreach ($decline as $item) {
            ...
            $project->com_offers->where('is_tongue', $is_tongue)
                ->where('option', $item['option'])->each->decline();
            $project->com_offers->where('is_tongue', $is_tongue)
                ->where('option',
                    $item['option'])->each->contracts->each->decline();
        }
    ...
    public function get_project_options()
    {
        ...
        $options = $project->work_volumes->whereIn('type', $types)
            ->pluck('option', 'id')->unique();
        $results[] = [
            'id' => 'disabled',
    ...
    public function updateTimeResponsibleUser(
        ProjectTimeResponsibleUserRequest $request
    ) {
        DB::beginTransaction();
    ...
    public function getProjectsForHumanAccounting(Request $request): array
    {
        ...
        return $projects->map(function ($project) {
            return [
                'code' => $project->id.'',
                'label' => $project->id.') '.$project->name_with_object,
            ];
        })->toArray();
    }
