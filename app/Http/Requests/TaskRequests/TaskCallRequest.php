...
class TaskCallRequest extends FormRequest
{
    ...
    protected function prepareForValidation(): void
    {
        if ($this->contractor_phone_number != null) {
            $this->merge([
                'contractor_phone_number' => preg_replace('~[\D]~', '',
    ...
    {
        return [
            'contractor_full_name'       => ($this->contractor_full_name != null
                    ? 'required' : 'nullable')
                .'|max:200|unique:contractors,full_name,'.($this->contractor_id
                    ? $this->contractor_id : ''),
            'contractor_short_name'      => 'required_with:contractor_full_name|max:100',
            'contractor_inn'             => 'nullable|string|min:10|max:14|unique:contractors,inn,'
                .($this->contractor_id ? $this->contractor_id : ''),
            'contractor_kpp'             => 'nullable|string|size:9|unique:contractors,kpp,'
                .($this->contractor_id ? $this->contractor_id : ''),
            'contractor_ogrn'            => 'nullable|string|min:12|max:15|unique:contractors,ogrn,'
                .($this->contractor_id ? $this->contractor_id : ''),
            ...
            'contractor_phone_number'    => 'nullable|string|min:11|max:11|unique:contractors,phone_number,'
                .($this->contractor_id ? $this->contractor_id : ''),
            'contractor_email'           => 'nullable|max:50|email|unique:contractors,email,'
                .($this->contractor_id ? $this->contractor_id : ''),
