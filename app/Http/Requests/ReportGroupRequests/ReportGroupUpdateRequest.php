...
class ReportGroupUpdateRequest extends FormRequest
{
    /**
    ...
    {
        return boolval(auth()->user()->hasPermission('human_resources_report_group_update'));
    }
    ...
    {
        if (ReportGroup::where('name', $this->request->get('name'))->where('id', '!=', $this->request->get('report_group_id'))->exists()) {
        ...
        if ($this->job_categories and ! $this->skip_job_categories_check) {
            ...
            $jobCategories = JobCategory::whereIn('id', $this->request->get('job_categories'))->where('report_group_id', '!=', $this->request->get('report_group_id'))->get();
    ...
    public function rules()
    {
        return [
            ...
            'deleted_job_categories' => ['nullable', 'array'],
            'deleted_job_categories.*' => ['required', 'exists:job_categories,id'],
        ];
    }
}
