...
class UserUpdateRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'group_id'              => Auth::user()->can('users_edit')
                ? 'required_if:can_edit,1|integer|exists:groups,id' : '',
            'company'               => Auth::user()->can('users_edit')
                ? 'required_if:can_edit,0|integer' : '',
            'birthday'              => 'nullable|max:10',
            'person_phone'          => 'nullable|string|min:11|max:11|unique:users,person_phone,'
                .$this->id,
            'work_phone'            => 'nullable|string|unique:users,work_phone,'
                .$this->id,
            'email'                 => $this->email
                ? ('required|max:100|email|unique:users,email,'.$this->id) : '',
            'password'              => $this->password
                ? 'min:7|regex:/(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])/' : '',
