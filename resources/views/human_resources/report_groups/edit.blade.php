    ...
    <div class="row">
        ...
        <div class="card col-lg-6 mr-auto ml-auto pd-0-min card-body-tech" id="base" v-cloak>
            <div class="card-header">
                <div class="row">
                    <div class="col-md-12">
                        <h4 class="h4-tech fw-500 mt-0">Редактирование отчётной группы</h4>
                    </div>
                </div>
            </div>
            <div class="card-body">
    ...
    <script>
        ...
        vm = new Vue({
            ...
            data: {
                ...
                name: '',
                reportGroup: {!! json_encode($data['report_group']) !!},
                jobCategories: [],
            ...
            created() {
                ...
                this.name = this.reportGroup.name;
                for (const jobCategory in this.reportGroup.job_categories) {
                    this.jobCategories.push({
                        id: String(this.reportGroup.job_categories[jobCategory].id),
                    });
                }
            },
            mounted() {
            ...
            methods: {
                submit() {
                    this.$refs.observer.validate().then(success => {
                        ...
                        payload.report_group_id = this.reportGroup.id;

                        const deletedJobCategories = this.reportGroup.job_categories.filter(el => this.jobCategories.map(cat => cat.id).indexOf(String(el.id)) === -1).map(el => String(el.id));
                        if (deletedJobCategories.length > 0) {
                            payload.deleted_job_categories = deletedJobCategories;
                        }

                        const jobCategoriesDiff = this.jobCategories.filter(el => this.reportGroup.job_categories.map(cat => String(cat.id)).indexOf(el.id) === -1).map(el => el.id);
                        if (jobCategoriesDiff.length > 0) {
                            payload.job_categories = jobCategoriesDiff;
                        }
                        ...
                        this.loading = true;
                        axios.put('{{ route('human_resources.report_group.update', $data['report_group']->id) }}', payload)
                            .then(response => {
                ...
                jobCategoryOptions(index) {
                    return this.jobCategories[index].id ? [this.allJobCategories.find(el => el.id === this.jobCategories[index].id)].concat(this.filteredJobCategories).filter(el => el) : this.filteredJobCategories;
                },
                ...
                confirm(ids, errorType) {
                    if (ids.length > 0) {
                        const labels = Object.entries(this.$refs)
                            ...
                            .filter(ref => ref[1][0] ? ids.indexOf(ref[1][0].value) !== -1 : false)
