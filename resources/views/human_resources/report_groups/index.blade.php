    ...
    <div class="row" id="base" v-cloak>
        <div class="col-md-12 mobile-card">
            <div aria-label="breadcrumb" role="navigation">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item active" aria-current="page">Отчётные группы</li>
                </ol>
            </div>
            ...
            <div class="card col-xl-10 mr-auto ml-auto pd-0-min">
                <div class="card-body card-body-tech">
                    <h4 class="h4-tech fw-500 m-0" style="margin-top:0"><span v-pre>Отчётные группы</span></h4>
                    <a class="tech-link modal-link d-block ml-1">
                        Найдено групп: @{{ totalItems }}
                    </a>
                    <div class="fixed-table-toolbar toolbar-for-btn" style="padding-left:0">
                        <div class="row">
                            ...
                            @can('human_resources_report_group_create')
                                <div class="col-md-7 text-right mt-10__mobile">
                                    <a href="{{ route('human_resources.report_group.create') }}" class="btn btn-sm btn-primary btn-round responsive-button">
                                        Создать отчётную группу
                                    </a>
                                </div>
                            @endcan
                        </div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover mobile-table table-fix-column">
                            ...
                            <tr>
                                ...
                                <th><span class="text-truncate d-inline-block">Численность, должн. групп</span></th>
                                <th></th>
                            </tr>
                            </thead>
                            <tbody>
                            <tr v-if="report_groups.length === 0">
                                <td>
                                    Нет данных
                                </td>
                                <td></td>
                            ...
                            <tr v-for="report_group in report_groups">
                                <td data-label="Наименование">
                                    @{{ report_group.name }}
                                </td>
                                <td data-label="Численность, должн. групп">
                                    @{{ report_group.job_categories_count }}
                                </td>
                                <td class="text-right actions">
                                    <a data-balloon-pos="up" :href="'{{ route('human_resources.report_group.show', 'report_group_id') }}'.split('report_group_id').join(report_group.id)"
                                    ...
                                    @can('human_resources_report_group_update')
                                        <a data-balloon-pos="up" :href="'{{ route('human_resources.report_group.edit', 'report_group_id') }}'.split('report_group_id').join(report_group.id)"
                                           aria-label="Редактировать" class="btn btn-link btn-xs btn-space btn-warning mn-0">
                                            <i class="fa fa-pencil"></i>
                                        </a>
                                    @endcan
                                    @can('human_resources_report_group_destroy')
                                        <button data-balloon-pos="up" aria-label="Удалить" class="btn btn-link btn-xs btn-space btn-danger mn-0"
                                                @click="removeReportGroup(report_group.id)">
    ...
    <script type="text/javascript">
        vm = new Vue({
            ...
            data: {
                ...
                totalItems: {!! json_encode($data['report_groups_count']) !!},
                currentPage: 1,

                report_groups: JSON.parse('{!! addslashes(json_encode($data['report_groups'])) !!}'),
            ...
            methods: {
                ...
                @can('human_resources_report_group_destroy')
                removeReportGroup(id) {
                    swal({
                        title: 'Вы уверены?',
                        text: "Отчётная группа будет удалена!",
                        type: 'warning',
                        ...
                        if (result.value) {
                            axios.delete('{{ route('human_resources.report_group.destroy', ['']) }}' + '/' + id)
                                .then(() => {
                                    this.report_groups.splice(this.report_groups.findIndex(el => el.id === id), 1);
                                    this.hideTooltips();
                                })
