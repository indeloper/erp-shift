    ...
    <div class="row">
        <div class="col-md-12 mobile-card">
            <div aria-label="breadcrumb" role="navigation">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">Редактирование бригады</li>
                </ol>
            </div>
        </div>
        <div class="card col-md-8 col-lg-6 col-xl-4 mr-auto ml-auto pd-0-min card-body-tech" id="base" v-cloak>
            <div class="card-header">
                <div class="row">
                    <div class="col-md-12">
                        <h4 class="h4-tech fw-500 mt-0">Редактирование бригады</h4>
                    </div>
                </div>
            </div>
            <div class="card-body">
    ...
    <script>
        ...
        vm = new Vue({
            ...
            data: {
                ...
                brigade: {!! json_encode($data['brigade']) !!},
                number: 1,
                direction: '',
                foreman: '',
            ...
            created() {
                this.searchForemen(this.brigade.foreman ? this.brigade.foreman.last_name : '');
                this.number = this.brigade.number;
                this.direction = String(this.brigade.direction);
                this.foreman = this.brigade.foreman ? this.brigade.foreman.id : '';
            },
            mounted() {
            ...
            methods: {
                submit() {
                    this.$refs.observer.validate().then(success => {
                        ...
                        const payload = {};
                        payload.brigade_id = this.brigade.id;
                        ...
                        this.loading = true;
                        axios.put('{{ route('human_resources.brigade.update', $data['brigade']->id) }}', payload)
                            .then(response => {
                ...
                confirm(ids, errorType) {
                    if (ids.length > 0) {
                        ...
                        const CUSTOM_ERROR_MESSAGES = {
                            'user_in_other_brigade': `Сотрудник ${label} уже состоит в составе существующей бригады.<br>
                                <br>Исключить его из состава существующей бригады и назначить бригадиром новой?`,
                            'foreman_in_other_brigade': `Сотрудник ${label} уже является бригадиром существующей бригады.<br>
