    ...
    <div class="row" id="base" v-cloak>
        <div class="col-md-12 mobile-card">
            <div aria-label="breadcrumb" role="navigation">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item active" aria-current="page">Бригады</li>
                </ol>
            </div>
                ...
                <div class="card-body-tech">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4 mt-10__mobile">
                            ...
                            <el-select v-if="filter_attribute === 'foreman_id'"
                            ...
                            <el-select v-else-if="filter_attribute === 'direction'"
                                       v-model="filter_value"
                                       clearable
                                       @change="updateCurrentCustomName"
                            ...
                            >
                                <el-option
                                    v-for="item in Object.entries(directions).map(el => ({ id: el[0], name: el[1] }))"
                                    :key="item.id"
                                    :label="item.name"
                                    :value="item.id">
                                </el-option>
            ...
            <div class="card col-xl-10 mr-auto ml-auto pd-0-min">
                <div class="card-body card-body-tech">
                    <h4 class="h4-tech fw-500 m-0" style="margin-top:0"><span v-pre>Бригады</span></h4>
                    <a class="tech-link modal-link d-block ml-1">
                        Найдено бригад: @{{ totalItems }}
                    </a>
                    <div class="fixed-table-toolbar toolbar-for-btn" style="padding-left:0">
                        <div class="row">
                            <div class="col-md-5">
                                <el-input placeholder="Поиск по номеру" v-model="search_tf" clearable
                                          class="responsive-button"
                                          prefix-icon="el-icon-search" id="search-tf" @clear="doneTyping"
                                          style="width: 270px"
                                ></el-input>
                            </div>
                            @can('human_resources_brigades_create')
                                <div class="col-md-7 text-right mt-10__mobile">
                                    <a href="{{ route('human_resources.brigade.create') }}" class="btn btn-sm btn-primary btn-round responsive-button">
                                        Создать бригаду
                                    </a>
                                </div>
                            @endcan
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover mobile-table table-fix-column">
                            <thead>
                            <tr>
                                <th><span class="text-truncate d-inline-block">Номер</span></th>
                                <th><span class="text-truncate d-inline-block">Направление</span></th>
                                <th><span class="text-truncate d-inline-block">Численность, чел.</span></th>
                                <th><span class="text-truncate d-inline-block">Бригадир</span></th>
                                <th></th>
                            </tr>
                            </thead>
                            <tbody>
                            ...
                            <tr v-for="brigade in brigades">
                                ...
                                <td class="text-right actions">
                                    ...
                                    @endcan
                                    @can('human_resources_brigades_destroy')
    ...
    <script>
        vm = new Vue({
            ...
            data: {
                ...
                PAGE_SIZE: 15,
                SINGLE_USE_FILTERS: [],
                totalItems: {!! json_encode($data['brigades_count']) !!},
                currentPage: 1,

                brigades: JSON.parse('{!! addslashes(json_encode($data['brigades'])) !!}'),
                directions: {!! json_encode($data['directions']) !!},

                search_tf: '',
                filter_attributes: [
                    {label: 'Направление', value: 'direction'},
                    {label: 'Бригадир', value: 'foreman_id'},
                ],
                filter_attribute: '',
                filter_value: '',
                filter_custom_name: '',
            ...
            created() {
                ...
                Object.entries(this.$route.query).forEach(entry => {
                    ...
                    if (entry[1] && this.filter_attributes_filtered.map(attr => attr.value).indexOf(field) !== -1) {
                        let customName = '';
                        if (field === 'foreman_id') {
                            this.promisedSearchForemen(value)//entry[1]
                                .then(() => {
                                    const foreman = this.foremen.find(el => {
                                        return el.id == value;
                                    });
                                    customName = foreman ? foreman.name : value;
                                    this.filters.push({
                            ...
                            const direction = this.directions[value],
                            customName = direction ? direction : '';
                            this.filters.push({
                                attribute: this.filter_attributes.find(el => el.value === field).label,
                                field: field,
                                value: value,
            ...
            methods: {
                doneTyping() {
                    ...
                    if (this.search_tf) {
                        ...
                        if (!count) {
                            queryObj['number'] = this.search_tf;
                ...
                updateFilteredBrigades() {
                    axios.post('{{ route('human_resources.brigade.paginated') }}', {url: vm.$route.fullPath, page: vm.currentPage})
                        .then(response => {
                            vm.brigades = response.data.data.brigades;
                            vm.totalItems = response.data.data.brigades_count;
                        })
                        .catch(error => console.log(error));
                },
                changePage(page) {
                    this.$router.replace({query: Object.assign({}, this.$route.query, {page: page})}).catch(err => {});
                    this.updateFilteredBrigades();
                },
                resetCurrentPage() {
                ...
                getFilterValueLabel(filter) {
                    switch(filter.field) {
                        case 'direction':
                        case 'foreman_id': return filter.custom_name;
                ...
                searchForemen(query) {
                    if (query) {
                        axios.get('{{ route('tasks::get_users') }}', {
                            ...
                            .then(response => this.foremen = response.data.results.map(el => {
                                el.name = el.text;
                                return el;
                            }))
                            .catch(error => console.log(error));
                    ...
                    return new Promise((resolve, reject) => {
                        if (query) {
                            axios.get('{{ route('tasks::get_users') }}', {
                                ...
                                .then(response => {
                                    this.foremen = response.data.results.map(el => {
                                        el.name = el.text;
                                        return el;
                                    });
                                    resolve(response);
                ...
                updateCurrentCustomName() {
                    switch (this.filter_attribute) {
                        case 'foreman_id':
                            ...
                            this.filter_custom_name = foreman ? foreman.name : '';
                            break;
                        case 'direction':
                            const direction = this.directions[this.filter_value]
                            this.filter_custom_name = direction ? direction : '';
                            break;
                    }
                },
                getDirectionName(id) {
                ...
                removeBrigade(id) {
                    swal({
                        ...
                        if (result.value) {
                            axios.delete('{{ route('human_resources.brigade.destroy', ['']) }}' + '/' + id)
                                .then(() => {
