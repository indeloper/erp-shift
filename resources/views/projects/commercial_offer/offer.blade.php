...
<html>
    <head>
        ...
        <!-- CSS Files -->
        <link href="{{ mix('css/offer.css') }}" rel="stylesheet" />
        ...
        <htmlpageheader name="page-header">
            <table>
                <tr>
                    ...
                    <td style="text-align: right; padding-right: 35px; font-size: 13px">
            ...
            <div class="main-content">
                        ...
                        <div class="contacts">
                            <p>
                                ...
                                @if($contact)
                                    ...
                                    <b>{{ $contact->last_name . ' ' . $contact->first_name . (isset($contact->patronymic) ? ' ' . $contact->patronymic : '') }}</b><br>
                                    @if($contact->email)<b>{{ $contact->email }}</b><br>@endif
                ...
                <div class="list-body">
                    <div style="margin:10px auto;">
                        ...
                        @else
                            <h6 class="h6-offer">Коммерческое предложение<br> на {{ $work_volume->project_name }}
                                «{{ $object->name }}»<br> по адресу: {{ $object->address }}@if($object->cadastral_number), на земельном участке с кадастровым номером {{ $object->cadastral_number }}@endif.
                            </h6>
                        @endif
                    </div>
                    <table class="kp-table">
                        ...
                        @foreach($works->groupBy('work_group_id')->sortKeys() as $id => $group)
                            ...
                            <tr>
                                <td colspan="6" class="td-head">{{ $work_group_count }}. {{ $work_groups[$id]}}</td>
                            </tr>
                            ...
                            <tr>
                                <td colspan="5" class="td-footer"> {{ ['Итого по шпунтовым работам:',
                            ...
                            @if($materials->where('work_group_id', $id)->first())
                                ...
                                @else
                                    ...
                                    @foreach($materials->where('work_group_id', $id)->groupBy('combine_id') as $combine_id => $combine_group)
                                        @if ($combine_id != '') {{-- if it's a combined pile, make header--}}
                                        @php $work_count++; $combine_count = 0 @endphp
                                        <tr>
                                            <td>{{ $work_group_count }}.{{ $work_count }}</td>
                                            <td style="text-align:left">
                                                Поставка свай {{ $materials->where('combine_id', $combine_id)->first()->combine_pile() }}
                                            </td>
                                            <td></td>
                                            <td></td>
                                            <td></td>
                                        ...
                                        @foreach($combine_group as $material)                                           {{-- iterates through group --}}
                                            @foreach($splits->where('man_mat_id', $material->manual_material_id) as $split)   {{-- find split for current material --}}
                                                @if (in_array(($split->type), [1,3]))
                                                    @php ($combine_id != '') ? $combine_count++ : $work_count++; @endphp{{-- magic with numbers: different for combined pile and simple --}}
                                                @endif
                                                ...
                                                @if($split->type == 4)
                                                    ...
                                                    <tr>
                                                        <td>{{ $work_group_count }}.{{$work_count . (($combine_id != '') ? ('.' . $combine_count) : '') . '.' . $split_siblings }}</td>
                                                        <td style="text-align:left">
                                                            Обеспечительный платеж за
                                                            {{ $split->parent->name }}
                                                        </td>
                        ...
                        <tr>
                            <td colspan="5" class="td-footer" style="background-color:rgb(150, 150, 150);">
                                Итого:</td>
                            <td class="td-total" style="background-color:rgb(150, 150, 150)">{{ number_format($result_cost, 2, ',', ' ') }}</td>
                        </tr>
                        ...
                        @if($splits->where('type', 4)->first())
                            ...
                            @php $work_group_count++; @endphp
                            <tr>
                                <td colspan="6" class="td-head">{{ $work_group_count }}. Возврат обеспечительного платежа:</td>
                            </tr>
                            ...
                            <tr>
                                <td colspan="5" class="td-footer">
                                    Итого по возврату обеспечительного платежа:</td>
                        ...
                        @if($splits->where('type', '2')->first())
                            ...
                            @php $work_group_count++; @endphp
                            <tr>
                                <td colspan="6" class="td-head">{{ $work_group_count }}. Обратный выкуп:</td>
                            </tr>
                        ...
                        @if($buyback or $security)
                            <tr>
                                <td colspan="5" class="td-footer" style="background-color:rgb(150, 150, 150);">
                                    ...
                                    {{ $security ? ' возврата обеспечительного платежа' : '' }}:</td>
                                <td class="td-total">{{ number_format($result, 2, ',', ' ') }}</td>
                            </tr>
                        @endif
                        <tr>
                            <td colspan="5" class="td-footer">
                                в том числе НДС {{ $offer->nds }}%:</td>
                ...
                <div>
                    ...
                    <htmlpagefooter name="page-footer">
                        <div class="list-footer" style="font-size: 10px; margin-top: 1mm; margin-left: 5mm; margin-right: 3mm; position: relative;">
