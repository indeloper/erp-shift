@if($com_offersForeach->isNotEmpty() or in_array(Auth::id(), [$respTongueKP, $respPileKP]))
    <div class="card">
        ...
        <div
                id="collapseCP"
                class="card-collapse collapse {{ !\Session::get('com_offer') ? "show" : '' }} @if(Request::has('task_16')) show @endif"
        >
                ...
                <div class="card strpied-tabled-with-hover">
                    <div class="fixed-table-toolbar toolbar-for-btn">
                        ...
                        @if ($respTongueKP == Auth::id())
                            <div class="pull-right">
                                <button
                                        class="btn-round btn-outline btn-sm add-btn btn"
                                        data-toggle="modal"
                                        data-target="#save-offer"
                                        onclick="upload_CO(1)"
                                >
                                    ...
                                    @foreach($offer_group_by_is_tongue as $offers_type)
                                        @foreach($offers_type as $key => $offer)
                                            @if($key == 0)
                                                <tr>
                                                    <td
                                                            data-label="Дата изменения"
                                                            data-toggle="collapse"
                                                            href=".o{{ hash('md5', $offer->option) . 'is_tongue-' . $offer->is_tongue }}"
                                                            role="button"
                                                            class="collapsed tr-pointer prerendered-date-time"
                                                            aria-expanded="false"
                                                            aria-controls="o{{ hash('md5', $offer->option) . 'is_tongue-' . $offer->is_tongue }}"
                                                    >
                                                        {{ $offer->created_at }}
                                                    </td>
                                                    <td
                                                    ...
                                                    <td
                                                            ...
                                                            href=".o{{ hash('md5', $offer->option) . 'is_tongue-' . $offer->is_tongue }}"
                                                            role="button"
                                                            class="collapsed tr-pointer text-center"
                                                            aria-expanded="false"
                                                            aria-controls="o{{ hash('md5', $offer->option) . 'is_tongue-' . $offer->is_tongue }}"
                                                    >
                                                        {{ $offer->version }}
                                                    ...
                                                    >
                                                        @if($type == 0)
                                                            @if($offer->status == 5 and ((isset($COpile) ? $COpile->user_id : 0) == Auth::user()->id))
                                                                <button
                                                                        rel="tooltip"
                                                                        class="btn-link btn-xs btn-success btn padding-actions mn-0"
                                                                        data-original-title='Подтвердить согласование КП с заказчиком'
                                                                        onclick="alertRespUser(this, {{ $offer->id }})"
                                                                >
                                                        ...
                                                        @if($offer->file_name)
                                                            <a
                                                                    ...
                                                                    class="btn-info btn-link btn-xs btn padding-actions"
                                                                    data-original-title='Посмотреть документ'
                                                            >
                                                                <i class="fa fa-eye"></i>
                                                            </a>
                                                        @endif
                                                        @if($offer->work_volume)
                                                            <a
                                                                    rel="tooltip"
                                                                    href="{{ route('projects::commercial_offer::card_' . ($offer->is_tongue == 2 ? 'double' : ($offer->is_tongue == 1 ? 'tongue' : 'pile')), [$project->id, $offer->id]) }}"
                                                                    class="btn-open btn-link btn-xs btn padding-actions"
                                                                    data-original-title="Открыть"
                                                            >
                                            ...
                                            @else
                                                <tr class="collapse o{{ hash('md5', $offer->option) . 'is_tongue-' . $offer->is_tongue }}">
                                                    ...
                                                    <td
                                                            data-label="Версия"
                                                            class="text-center"
                                                    >{{ $offer->option ? $offer->option: ' id: ' . $offer->id }}</td>
                                                    <!-- <td>{{ $offer->last_name }} {{ $offer->first_name }} {{ $offer->patronymic }}</td> -->
                                                    <td data-label="Статус">{{ $offer->com_offer_status[$offer->status] }}</td>
                                                    <td
                                                            data-label="Заявки"
                                                            class="text-center"
                                                    >{{ $offer->get_requests->count() }}</td>
                                                    <td
                                                            data-label=""
                                                            class="text-right actions"
                                                    >
                    ...
                    <div class="fixed-table-toolbar toolbar-for-btn">
                        ...
                        @if(!$work_volumes_all->where('type', 0)->where('status', 2)->isEmpty() || !$work_volumes_all->where('type', 1)->where('status', 2)->isEmpty())
                            @if ((in_array(Auth::id(), $allRespUsers) or !isset($contractLogic)) and !in_array(Auth::id(), $project->respUsers->whereIn('role', [5, 6])->pluck('user_id')->toArray()))
                                <div class="pull-right">
                                    <button
                                            class="btn-success btn-round btn-outline btn-sm add-btn btn"
                                            style="margin-top:25px;"
                                            data-toggle="modal"
                                            data-target="#create_offer_request"
                                    >
                                        Новая заявка
                                    </button>
                                </div>
                                    ...
                                    <tr>
                                        ...
                                        >
                                            <button
                                                    ...
                                                    data-toggle="modal"
                                                    data-target="#view-request-offer{{ $offer_request->id }}"
...
@foreach ($commercial_offer_requests as $offer_request)
    <div
            ...
            id="view-request-offer{{ $offer_request->id }}"
            role="dialog"
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-9">
                                        ...
                                        @foreach($offer_request->files->where('is_result', 0)->where('is_proj_doc', 0) as $file)
                                            <a
                                                    target="_blank"
                                                    href="{{ asset('storage/docs/commercial_offer_request_files/' . $file->file_name) }}"
                                            >
                                                {{ $file->original_name }}
                                            </a>
                                            <br>
                                        ...
                                        @foreach($offer_request->files->where('is_result', 0)->where('is_proj_doc', 1) as $file)
                                            <a
                                                    target="_blank"
                                                    href="{{ asset('storage/docs/project_documents/' . $file->file_name) }}"
                                            >
                                                {{ $file->original_name }}
                                            </a>
                                            <br>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="card border-0">
                    <div class="card-body ">
                        <form
                                id="create_offer_request_form"
                                class="form-horizontal"
                                action="{{ route('projects::commercial_offer::requests::store', $project->id) }}"
                                method="post"
                                ...
                                @if(!$work_volumes_all->where('type', 0)->where('status', 2)->isEmpty())
                                    <div class="row">
                                        <div class="col-md-12">
                                            ...
                                            >
                                                <div class="row">
                                                    <label class="col-sm-3 col-form-label">Объем работ
                                                        <star class="star">*</star>
                                                    </label>
                                                    <div class="col-sm-9">
                                                        <select
                                                                name="tongue_offer_id"
                                                                id="select-com_offer_tongue"
                                                                style="width:100%"
                                                                required
                                                        >
                                                ...
                                                <div class="row">
                                                    ...
                                                    <div class="col-sm-9">
                                                        <div class="form-group">
                                                            <textarea
                                                                    class="form-control textarea-rows"
                                                                    name="descriptionTongue"
                                                                    maxlength="500"
                                                            ></textarea>
                                                        </div>
                                                    </div>
                                                        ...
                                                        <div class="file-container">
                                                            ...
                                                            <div class="file-upload ">
                                                                <label class="pull-right">
                                                                    ...
                                                                    <input
                                                                            type="file"
                                                                            name="tongue_documents[]"
                                                                            accept="*"
                                                                            id="uploadedOfferPileTongueFiles"
                                                ...
                                                <div class="row">
                                                    ...
                                                    <div class="col-sm-9">
                                                        <select
                                                                class="js-select-proj-doc"
                                                                name="project_documents_tongue[]"
                                                                data-title="Выберите документ"
            ...
            <div class="modal-footer">
                ...
                <button
                        id="submit_offer_request"
                        type="submit"
                        form="create_offer_request_form"
                        class="btn btn-info"
                >Сохранить
                </button>
            </div>
...
<div class="d-none">
    <form
            id="doubleKP"
            target="_blank"
            action="{{ route('projects::commercial_offer::create_double_kp') }}"
            method="POST"
    >
        @csrf
        <input
                ...
                type="hidden"
                name="firstKP"
                value="{{ isset($com_offers->where('is_tongue', 1)->where('status', 5)->first()->id) ? $com_offers->where('is_tongue', 1)->where('status', 5)->first()->id : 0 }}"
        >
        <input
                id="secondKP"
                type="hidden"
                name="secondKP"
                value="{{ isset($com_offers->where('is_tongue', 0)->where('status', 5)->first()->id) ? $com_offers->where('is_tongue', 0)->where('status', 5)->first()->id : 0 }}"
        >
    </form>
</div>
...
@foreach($agree_tasks as $agree_task)
    <div
            class="modal fade bd-example-modal-lg show"
            id="agreeKP{{ $agree_task->target_id }}"
            role="dialog"
            aria-labelledby="modal-search"
            style="display: none;"
    >
        ...
        >
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="card border-0">
                        <form
                                id="form_solve_task{{ $agree_task->target_id }}"
                                class="form-horizontal"
                                action="{{ route('tasks::solve_task', $agree_task->id) }}"
                                method="post"
                        >
                            @csrf
                            <input
                            ...
                            <div class="form-group">
                                <div class="row">
                                    <label class="col-sm-3 col-form-label">Результат
                                        <star class="star">*</star>
                                    </label>
                                    <div class="col-sm-6">
                                        <div class="form-group">
                                            <select
                                                    id="status_result{{ $agree_task->target_id }}"
                                                    name="status_result"
                                                    offer_id="{{ $agree_task->target_id }}"
                                                    class="selectpicker"
                            ...
                            <div
                                    ...
                                    id="comment{{ $agree_task->target_id }}"
                            >
                                <div class="row">
                                    ...
                                    <div class="col-sm-6">
                                        <div class="form-group">
                                            <textarea
                                                    id="result_note{{ $agree_task->target_id }}"
                                                    class="form-control textarea-rows"
                                                    maxlength="300"
                                                    name="final_note"
                            ...
                            <div
                                    id="fromСontainer{{ $agree_task->target_id }}"
                                    class="row d-none"
                            >
                                <label class="col-sm-3 col-form-label">Перенести на дату
                                    <star class="star">*</star>
                                </label>
                                <div class="col-sm-3">
                                    <div class="form-group">
                                        <input
                                                id="from"
                                                name="revive_at"
                                                class="form-control datepicker task-transfer"
                                                autocomplete="off"
                                        >
                                    </div>
                                </div>
                ...
                <div class="modal-footer">
                    ...
                    <button
                            id="sendForm{{ $agree_task->target_id }}"
                            form="form_solve_task{{ $agree_task->target_id }}"
