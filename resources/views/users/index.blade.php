            ...
            >
                ...
                >
                    <li class="nav-item @if (Request::is('users')) show active @endif">
                        <a
                                class="nav-link link-line @if (Request::is('users')) active-link-line @endif"
                                href=""
                        >
                            Список сотрудников
                        </a>
                    ...
                    @can('users_permissions')
                        <li class="nav-item show">
                            <a
                                    class="nav-link link-line"
                                    href="{{ route('users::department_permissions') }}"
                            >
                                Группы прав
                            </a>
                        </li>
                ...
                <div class="fixed-table-toolbar toolbar-for-btn text-center text-sm-left">
                    ...
                    @if(strlen(Request::get('search')) > 0)
                        <a
                                ...
                                role="button"
                                class="btn btn-secondary btn-sm btn-outline m-0 d-block d-sm-inline-block"
                        >Сброс</a>
                    @endif
                    @can('users_create')
                        <div class="pull-right float-none float-sm-right">
                            <a
                                    class="btn btn-round btn-outline btn-sm add-btn d-block d-sm-inline-block"
                        ...
                        <table class="table table-hover mobile-table">
                            ...
                            @foreach($users->where('id', '!=', 1) as $user)
                                <tr
                                        ...
                                        class="href"
                                        data-href="{{ route('users::card', $user->id) }}"
                                >
                                    ...
                                    <td data-label="Компания">{{ isset($companies[$user->company]) ? $companies[$user->company] : '-' }}</td>
