                                ...
                                <div id="description">
                                    ...
                                    @if($contractor ?? false)
                                        @if ($contractor->projects()->count() > 0)
                                            <p><b>
                                                    Вместе с контрагентом будет удалено проектов:
                                                    <a title="Перейти к просмотру проектов" target="_blank" href="{{ route('projects::index', ['search' => $contractor->short_name]) }}">{{( $contractor->projects()->count()) }}</a>
                                                    . А также все входящие в них коммерческие предложения, объемы работ и договоры
                                                </b>
                                            </p>
                                        @endif
                                    @endif
                                    @if(!$task->is_solved)
                                        @if(Auth::id() == $task->responsible_user_id)
                                            <form id="solve_remove" action="{{ route(($task->contractor_id ? 'contractors::solve_remove' : 'building::mat_acc::delete_part_operation'), $task->id) }}" class="form-horizontal" method="post">
                                                @csrf
