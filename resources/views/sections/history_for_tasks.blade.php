...
{{--also in controller TaskValuationController method valuation--}}
    ...
    @foreach($solved_tasks as $solved_task)
        <tr class="@if(!$solved_task->is_solved)
            ...
            <td data-label="Исполнитель">
                @if($solved_task->responsible_user_id)
                    <a href="{{ route('users::card', $solved_task->responsible_user_id) }}" class="table-link">
            ...
            <td data-label="Автор">
                @if($solved_task->user_id)
                    <a href="{{ route('users::card', $solved_task->user_id) }}" class="table-link">
        ...
        <!-- Модалка события -->
        <div class="modal fade bd-example-modal-lg" id="show_task_info{{ $solved_task->id }}" role="dialog" aria-labelledby="modal-search" aria-hidden="true">
                                ...
                                <div class="right-bar-info">
                                    ...
                                    <div class="right-bar-info__item" style="margin-bottom:0;">
                                        ...
                                        <div class="task-info__text-unit">
                                            ...
                                            @if($solved_task->user_id)
                                                <a class="task-info__link task-info__body-title" href="{{ route('users::card', $solved_task->user_id) }}" style="font-size:14px">
                                                    {{ $solved_task->user->full_name }}
                                                </a>
                                            @else
                                                <span class="task-info__body-title">
                                    ...
                                    <div class="right-bar-info__item" style="margin-bottom:0;">
                                        @if($solved_task->status > 2)
                                            <div class="task-info__text-unit">
                                                ...
                                                <span class="task-info__body-title" @if($solved_task->project_id) href="{{ route('projects::card', $solved_task->project_id) }}" @endif>{{ $solved_task->object_address ? $solved_task->object_address : 'Проект не указан' }}</span>
                                            </div>
                                        @endif
                                        <div class="task-info__text-unit">
                                            <span class="task-info__head-title">
                                            ...
                                            <span class="task-info__body-title">
                                                <a @if($solved_task->project_id) href="{{ route('projects::card', $solved_task->project_id) }}" @endif class="task-info__link task-info__body-title" target="_blank">{{ $solved_task->project_name ? $solved_task->project_name : 'Проект не выбран' }}</a>
                                            </span>
                                        </div>
                                        <div class="task-info__text-unit">
                                            <span class="task-info__head-title">
                                            ...
                                            <a class="task-info__link task-info__body-title" @if($solved_task->contractor_id) href="{{ route('contractors::card', $solved_task->contractor_id) }}" @endif>{{ $solved_task->contractor_name ? $solved_task->contractor_name : 'Контрагент не выбран' }}</a>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="left-bar-main">
                                    ...
                                    <div class="row">
                                        <div class="col-md-12">
                                            ...
                                            @endif
                                            @if($solved_task->status == 2)
                                                @if($solved_task->description)<p class="task-info__ml">{{ $solved_task->description }}</p>@endif
                                                @if($solved_task->status == 2)
                                                    ...
                                                    @elseif($solved_task->incoming_phone && !$local_contact)
                                                        <p style="font-size:14px;">Звонок с телефона: {{ $solved_task->incoming_phone }}</p>
                                                    @else
                                                        ...
                                                        <div class="left-bar-main_unit">
                                                            ...
                                                            @if($solved_task->status > 2 and $work_volumes->whereIn('status', [1,2]))
                                                                @foreach($work_volumes->whereIn('status', [1,2])->whereIn('type', [0,1]) as $wv)
                                                                    <a class="task-info_file" target="_blank" href="{{ route('projects::work_volume::card_' . ($wv->type ? 'pile': 'tongue') , [$solved_task->project_id, $wv->id]) }}">Карточка объема работ {{ $wv->type ?'(Свая)': '(Шпунт)' }}</a>
                                                                @endforeach
                                                            @endif
                                                            @if($solved_task->status > 4 and $com_offers->whereIn('status', [1,2,4]))
                                                                @foreach($com_offers->whereIn('status', [1,2,4]) as $offer)
                                                                    <a class="task-info_file" target="_blank" href="{{ route('projects::commercial_offer::card_'. ($offer->is_tongue ? 'tongue' : 'pile'), [$solved_task->project_id, $offer->id]) }}">Карточка коммерческого предложения {{ $offer->is_tongue ? '(Шпунт)' : '(Свая)' }}</a>
                                                                @endforeach
                                                            @endif
                                                            @if(in_array($solved_task->status, $solved_task::CONTR_STATUS))
                                                                <a class="task-info_file" target="_blank" href="{{ route('projects::contract::card', [$solved_task->project_id, $solved_task->target_id]) }}">
                                                                    Карточка договора
                                                                </a>
                                                            @endif
                                                        </div>
                                            ...
                                            <div class="row">
                                                ...
                                                <div class="col-sm-9">
                                                    <p style="padding-left:20px;">
                                                        Предыдущий исполнитель : {{ $task_responsible_users->where('id', $task_redirect->old_user_id)->first()->long_full_name }}
                                                    <p style="padding-left:20px;">
                                                        Новый исполнитель : {{ $task_responsible_users->where('id', $task_redirect->responsible_user_id)->first()->long_full_name }}
                                                    </p>
                                            ...
                                            <div class="col-md-12">
                                                ...
                                                @if($solved_task->status > 2)
                                                    @if(in_array($solved_task->status, [3,4,5,7]))
                                                        <p class="task-info__ml" > <i>{!! ($solved_task->revive_at ? "Задача была перенесена на  <b>" . date('d.m.Y' ,strtotime($solved_task->revive_at)) . '</b>' : 'Задача решена') !!} </i></p>
                                                        <br>
