...
<script>
  $(function () {
    ...
    let materialsStandardsAddingForm = $('#materialsStandardsAddingForm').dxForm({
          ...
          items: [
              ...
              items: [
                {
                  ...
                  editorOptions: {
                    ...
                    onItemDeleted: function (e) {
                      ...
                      selectedMaterialsList.option('items').forEach(function
                        (selectedItem) {
            ...
            ,
            {
              ...
              buttonOptions: {
                ...
                onClick: function () {
                  let
                    selectedMaterialsData = materialsStandardsAddingForm.getEditor('selectedMaterialsList').option(
                      'items');
                  ...
                  selectedMaterialsData.forEach(function (material) {
                    ...
                    let
                      validationUid = getValidationUid(
                        material.standard_id, material.accounting_type, quantity, amount, material
                          .comment_id);
                    console.log(`added material`, material);
                    transferMaterialDataSource.store().insert(
                      {
                        id:
                          'uid-' + new DevExpress.data.Guid().toString(),
                        standard_id:
                ...
                materials.standard.incriminate-selection-counter')}}",
                      data;
                  :
                    JSON.stringify({ standardId: material.standard_id });
                  })
                    ;
      ...
      let m;
      ainer = $('#commentPopupContainer').dxPopup({
        height: 'auto',
        width: 'auto',
      ...
      let
            ...
            buttons: [
              ...
              },
              {
                ...
                template: (container, options) => {
                  ...
                  let commentIconClass = !options.data.comment && !options.data.initial_comment ?
                        ...
                        .click(() => {
                          ...
                          if (commentData.comment) {
                            materialCommentEditForm.getEditor(
                              'materialCommentTextArea').option('value', commentData.comment);
                          } else {
                            if (commentData
                              .
                          i {
        ...
        >${options.text}</div>`)
          ...
          standard-quantity="${Math.round(
                    options.data.quantity * 100) / 100}" accounting-type="${options.data.accounting_type}" initial-comment-id="${
                    options.data.initial_comment_id}"></div>`)
                    .appendTo(container);
          ...
          },
          {
            ...
            cellTemplate:
              function (
                container, options) {
                let quantity = Math.round(options.data.quantity * 100)
                  / 100;
                if (quantity !== null) {
                  $(`<div>${quantity} ${options.data.measure_unit_value}</
      div>`)
                    .appendTo(container);
                } else {
                  $(`<div class
          ="
            measure-units-only">${
                    options.data.measure_unit_value}</div>`)
                    .appendTo(
                      container);
                }
              },
        ...
        onEditorPreparing: (e) => {
          if (e.dataField === 'quantity' && e.parentType === 'dataRow' && transferOperationInitiator === 'source') {
            if (e.row.data.accounting_type
              === 2) {
              e.cancel = true;
              e.editorElement.append($(`<div>${e.row.data.quantity} ${e.row.data.measure_unit_value}</
                        div>`));
            }
          }
        },
        onRowUpdating: (e) => {
          e.newData.validationUid =
            getValidationUid(e.oldData.standard_id, e.oldData.accounting_type, e.oldData.quantity, e.newData.amount, e.oldData.initial_comment_id);
          ...
          items: [
            {
              ...
              nged;
                  ...
                  if (getTransferMaterialGrid().getDataSource().items().length > 0 && e.previousValue
                    !== null) {
                    let confirmDialog = DevExpress
                      .ui.dialog.confirm('При смене
                    объекта;
                    отправления;
                    будут;
                    удалены;
                  ...
                  :
                    ...
                    function saveOperationData() {
                      ...
                      if (
                        ...
                        || transferOperationInitiator ===
                        'source') {
                        transferOperationData.operation_date = new Date(
                          operationForm.option('formData').operation_date_start).toJSON().split('T')[0];
                      }
                      ...
                      if (transferOperationInitiator === 'destination') {
                        transferOperationData.operation_date =
                          ...
                          Date(operationForm.option(
                            'formData').operation_date_end).toJSON().split
                          ('T')[0];
                      }

                      transferOperationData.source_responsible_user_id = operationForm.option('formData').source_responsible_user_id;
                      transferOperationData.destination_responsible_user_id = operationForm.option('formData').des;
                      o;
                      _id;
                      ...
                      transferOperationData.materials =
                        t;
                      Source.store().createQuery().toArray();
                  ...
                  materials.operations.transfer.validate-material-list')}}",
                    ...
                    :
                      'json',
                        data;
                    :
                      JSON.stringify(transferOperationData),
                        success;
                    :
                      ...
                      function (e) {
                        ...
                        e.validationResult.forEach((validationElement,
                          ...
                          let validatedData =
                            transferMaterialDataSource.store().createQuery()
                              ...
                              case 'validated':
                                if (validationResult === 'valid') {
                                  ...
                                  if (!materialErrorList
                                    [element.validationUid].errorList) {
                                    return;
                                  }
    ...
    materials.index')}}/?project_object=' + desti;
                      ...
                      transferMaterialStore.byKey(editedRowKey)
                        .done(
                          function (dataItem) {
                            ...
                            let
                              calculatedAmount = dataItem.total_amount
                            ;
                            let i;
                            ntId =
      ...
      standard-id='${dataItem.standard_id}'][standard-quantity='${dataItem.quantity}'][initial-comment-id='${
                                  dataItem.i
