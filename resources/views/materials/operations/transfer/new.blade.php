  ...
  $(function () {
            ...
            "Выбранные материалы",
              items: [
                ...
                standardItem);
                  ...
                  materialsStandardsList = materialsStandardsAddingForm.getEditor("materialsStandardsList");
                      let
            s
          sList =
        materi
          ...
          lsList");
                ...
                ,
            {
              ...
              buttonOptions: {
                ...
                onClick: function () {
                  let
                selectedMaterialsData = materialsStandardsAddingForm.
                getEditor("selectedMaterialsList").option(
                  "items");
                  ...
                  material.standard_id, material.
                  accounting_type, quantity, amount, material
                  .comment_id);
                ...
                Guid().
                  ...
                  material.standard_name,
                      accounting_type: material.
                  accounting_type,
                      material_type: material.material_type,
                      measure_unit: material.measure_unit,
                measure_unit_value: material.
      ...
      headers: {
        ...
        "csrf-token"]'
            ...
            attr('content')
              ...
              dataType: "json",
                      url: "{{route('
                materials.standard.incriminate-selection-counter')}}",
                      data: JSON.stringify({ standardId: material.standard_id })
                    });

                    validateMaterialList(false,
              false, validationUid);
                  });
        ...
        let
          ...
          title:
              ...
              "Выберите материалы для добавления"
        });

        let m
                  ainer = $("#commentPopupContainer").
                  dxPopup({
          height: "auto",
          width: "auto",
        ...
        //<editor-fold
              ...
              transferMaterialColumns = [
          {
            ...
            buttons: [
              ...
              validationUid = options.data.validationUid;
                  let
          validationDiv =
            $('<div class="
            row-validation-indicator"/>')
              ...
              validationUid)
                  ...
                  updateRowsValidationState([options.data], options.
                data.validationState, options.data.

              validationResult, validationDiv);
                ...
                onClick: function (e) {
                  ...
                  let
                    ...
                    e.row.data, {
                      id:
                      clonedItemId,
                      validationUid: getValidationUid(e.row.data.
                          standard_id, e.row.data.accounting_type, e.row.data.quantity, e.row.data.amount, e.row.data.initial_comment_id)
                    }
                  );

                  transferMaterialDataSource.store().insert(clonedItem).done(() => {
                      transferMaterialDataSource.reload();
                    validateMaterialList(false, false, clonedItem.
                    validationUid)
                    ;
              ...
              },
              {
                ...
                template: (container, options) => {
                  ...
                  if (options.data.accounting_type) {
                      accountingType = options

                      .data.accounting_type;
                  }

                  let commentIconClass = !options.data.comment && !options.
                          data.initial_comment ?
                            "far fa-comment" : "fas fa-comment"
                          ;
                  ...
                  let commentLink;
                    ...
                    case 2:
                      commentLink = $("<
                        ...
                        .addClass("dx-link dx-icon " + commentIconClass +
                        " dx-link-icon")
                        .click(() => {
                          commentData = options.data;
                          if (commentData.comment) {
                        materialCommentEditForm.getEditor(
                        "materialCommentTextArea").
                  option("value", commentData.
                comment);
                          }
              else {
                            if (commentData
      ...
      ent) {
        ...
        materialCommentEditForm.getEditor("materialCommentTextArea").option("value", "")
          ...
          #commentPopupContainer").dxPopup("show");
                        ...
                        .mouseenter(function () {
                          let comment = "";
                          if (!options.data.comment && !options.data.
              initial_comment) {
                            return;
                          }
              ...
              комментарий:</b><br>${options.data.initial_comment}`
                          }
      ...
      default:
        ...
        >${options.text}</div>`)
          ...
          appendTo(divStandardName);
                ...
                if (options.data.comment) {
                  $(`<div class="material-comment"
              >${options.data.
          comment}</
            div>`)
                    .appendTo(
              divStandardName);
        divStandardName.addClass
      (
        "standard-name-cell-with-comment");
                }
        if (transferOperationInitiator !==
        "destination") {
                  let divStandardRemains = $(`<div class="
          standard-remains" standard-id="${options.data.standard_id}"

          standard-quantity="${Math.round(
            options.data.
          quantity * 100) / 100}" accounting-type="${options.

          data.accounting_type}" initial-comment-id="${
          options.data.
        initial_comment_id}"></div>`)
                    .
          appendTo(container);

                  divStandardRemains
          .mouseenter(function () {
                    let standardRemainsPopover = $('
              #standardRemainsTemplate');
      standardRemainsPopover.
      dxPopover(
        {
        position: "top",
                      width: 300,
        contentTemplate: "Остаток материал
        отправления",
                      hideEvent: "mouseleave",
          })
                      .dxPopover(
      ...
      alDataSource,
          ...
          onEditorPreparing: (e) => {
            if (e.dataField === "quantity" && e.parentType === "dataRow" && transferOperationInitiator === "source") {
              if (e.row.data.accounting_type
                    === 2) {
                e.cancel = true;
                e.editorElement.append($(`<div>${e.row.
                        data.quantity} ${e.row.data.
                      measure_unit_value}</
                        div>`))
              }
            }
          ...
          onRowUpdating: (e) => {
            e.newData.
                    validationUid =
                  getValidationUid(e.
                    oldData.standard_id, e.oldData.
                accounting_type, e.oldData.
              quantity, e.newData.amount, e.oldData.
                  initial_comment_id);
            e.
                  newData.validationState = "unvalidated";
            e.newData.
              validationResult = "none";
          },
              onRowUpdated: (e) => {
              recalculateStandardsRemains(e.key);
              validateMaterialList(false, false, e.
              data.validationUid);
          },
              onRowRemoved: (e) => {
            validateMaterialList(false, false, e.
        ...
        let operationForm = $
          ...
          items: [
            ...
            dataSource: {
                      store:
            projectObje
            cipatesInMaterialAccountingStore
                    },
            ...
            sourceProjectObjectId : null,
                    onValueChanged: function (
            e) {
                      function
            updateComponentsDataSources(
            sourceProjectObjectIdValue) {
            ...
            transferMaterialStore.clear();
              ...
              dsListDataSource() {
                        let dataGrid =
              materialsStandardsAddingForm.getEditor(
              "materialsStandardsList")
                        dataGrid.option(
                  "dataSource").reload();
                      }
              ...
              previousValue;
                let currentValue = e.value
                  ;
                      ...
                      if (getTransferMaterialGrid().getDataSource().items().length > 0 && e.
              previousValue
          !== null) {
                        let confirmDialog = DevExpress
          .ui.dialog.
          confirm('При смене
          объекта
              отправления будут удалены введенные данные по материалам
              операции.<br>
              Продолжить?', 'Смена объекта отправления');
              ...
              dialogResult) {
                ...
                rces(currentValue);
                            ...
                            suspendSourceObjectLookupValueChanged =
                true
                ;
              ...
              "required",
                      message:
              'Поле "Объект отправления" обязат
              я'
                    }
                  ]
                },
                  ...
                  validationRules: [
                    {
                      type:
            transfe
            or !== "destination" ? "required" : "",
                      message:
              'Поле "Дата отправления" обязательно для заполнения'
                    }
                  ]
                },
                // {
            ...
            //
              ...
              ля заполнения'
                ...
                {
                  name:
                  "sourceResponsibleUserSelectBox",
                  colSpan:
              transferOperationInitiator
          ===
        "destination" ? 3 : 2,
                  dataField:
          "source_responsible_user_id",
                  label: {
                    text: "Ответственный"
                  },
              editorType: "dxSelectBox",
              ...
              dataSource: {
                      store:
              usersWithMaterialListAccessStore
                    },
              ...
              items: [
                ...
                "destination_project_object_id",
                  ...
                  editorOptions: {
                    dataSource: {
                      store:
                projectObjectsListWhichParticipatesInMaterialAccountingStore
                    },
                displayExpr: "short_name",
                    valueExpr: "id",
            searchEnabled: true,
                    value:
              transferOperationInitiator === "destination" ? destinationProjectObjectId : null,
                  },
                  validationRules: [
                    {
                      type: "required",
                      message:
                'Поле "Объект получения" обязательно для заполнения'
                    }
                  ]
                },
                {
                  name:
                "operationDateEndDateBox",
                ...
                transferOperationInitiator === "destination",
                  ...
                  editorType: "dxDateBox",
                  editorOptions: {
                    value: transferOperationInitiator ===
                "destination" ? Date.now() : null,
                    max: Date.now(),
                    min: getMinDate(),
                  },
                  validationRules: [
        ...
        onInitiator ===
          ...
          2,
                  dataField:
          "destination_responsible_user_id",
          label: {
                    text:
            "Ответственный"
                  },
            ...
            editorOptions: {
              ...
              ore
                    ...
                    valueExpr: "id",
              searchEnabled: true,
                    value: transferOperationInitiator === "destination" ? {{$currentUserId}} :
                null
                  },
                  validationRules: [
            {
                      type: "required",
                      message: 'Поле "Ответств
    ...
    "Комментарий",
      ...
      '<div id="dropzone-external-1" class="dx-uploader-flex-box dx-theme-border-color dropzone-external">' +
                    ...
                    '<span class="dx-uploader-span">Фото ТН</
      ...
      "file-uploader" purpose="consignment-note-photo" index="1"></div>'
                },
                {
                  colSpan: 1,
                  template: '<div id="dropzone-external-2" class="

      dx-uploader-flex-box dx-theme-border-color dropzone-external">' +
                    '<img id="dropzone-image-2" class="dropzone-image" src="#"

      hidden alt="" />' +
                    '<div id="dropzone-text-2" class="
        dx-uploader-flex-box dropzone-text">' +
                    '<span class="dx-uploader-span">Фото машины спереди</span>' +
                    '</div>' +
                    '<div id="upload-progress-2" class="
      upload-progress"></div>' +
                    '</div>' +
                    '<div class="

      file-uploader" purpose="
    frontal-vehicle-photo" index="2"></div>'
                },
                {
                  colSpan: 1,
                  template: '<div id="

      dropzone-external-3" class="
            dx-uploader-flex-box dx-theme-border-color dropzone-external">' +
                    '<img id="dropzone-image-3" class
          ="
            dropzone-image" src="#" hidden alt="" />' +
                    '<div id="
              dropzone-text-3" class="
              dx-uploader-flex-box dropzone-text">' +
                    '<span class="
              dx-uploader-span">Фото машины сзади с материалами</span>' +
                    '</
          div>' +
                    '<div id="
          upload-progress-3" class
            ="upload-progress"></div>' +
                    '</div>' +
                    '
            <div class="
      ...
      purpose="behind-vehicle-photo" index="3"></div>'
                }
              ]
            },
            {
    ...
    ремещение",
      ...
      template: function (data, container) {
                  $("
        <div class='button-loading-indicator'></div><span class='
          dx-button-text'>" + data.text + "</span>").appendTo(
          container);
      ...
      setButtonIndicatorVisibleState(
        "createTransferOperation", true)
                  setElementsDisabledState(true);

                  let
        comment =
          operationForm.option("formData").new_comment;
                  if (!comment) {
                    let
        confirmDialog = DevExpress.
        ui.dialog.confirm('Вы не заполнили поле
        "Комментарий".<br>Продолжить
          без заполнения?', 'Ком
        ...
        function saveOperationData() {
          ...
          transferOperationData.
            ...
            ator = transferOperationInitiator;
              transferOperationData.source_project_object_id = operationForm.option(
            "formData").
              source_project_object_id;
          transferOperationData.
              destination_project_object_id = operationForm.option("formData").destination_project_object_id;
          // transferOperationData.material_operation_reason_id = operatio
              ata").material_operation_reason_id;
          transferOperationData.new_comment =

            operationForm.option("formData").new_comment;
          //TODO Дата формаируетс
               учитывать это при перобразовании, либо хранить в UTC в БД
          if (
          ...
          operationForm.option("formData").
            operation_date_start).toJSON().split("T")[0];
          }

          if (transferOperationInitiator === "destination") {
            transferOperationData.operation_date =
          new

          Date(operationForm.option(
            "formData").
              operation_date_end).toJSON().split
            ("T")[0];
          }

          transferOperationData.
          source_responsible_user_id = operationForm.option("formData").source_responsible_user_id;
          transferOperationData.destination_responsible_user_id = operationForm.
          option("formData").des
        o
      _id;

    transferOperationData.consignment_note_number = o
      m.option("formData").consignment_note_number;

          let uploadedFiles = []
          $(".file-uploader").each(
          function () {
            if ($(this).attr(
            "uploaded-file-id") !== undefined) {
            uploadedFiles.push($(this).attr("uploaded-file-id"));
            }
          })
              ;

          transferOperationData.uploaded_files =
              uploadedFiles;
              transferOperationData.materials =
                t
              Source.store().
              createQuery().toArray();
        ...
        function
                ...
                getValidationUid(standardId, accountingType, quantity, amount,
          ...
          switch (accountingType) {
            case 2:
              if (!quantity || !amount) {
                  return "uid-" + new DevExpress.data.Guid
                ().
                  toString();
              } else {
              filterConditions = [
      ...
      ,
          ...
          if (filteredData.length > 0) {
            ...
            return "uid-" + new
                DevExpress.data.Guid().toString();
          }
        }

        function
                  validateMaterialList(saveEditedData,
                showErrorWindowOnHighSeverity,
                validationUid) {
          let validationData;
          if (validationUid && !(saveEditedData)) {
            validationData = transferMaterialDataSource.store
                    ().createQuery()
              .filter(['validationUid', '=', validationUid])
              .toArray();
          }
                    else {
            validationData = transferMaterialDataSource.
                store().
                  createQuery()
              .toArray()
                    ;
          ...
          updateRowsValidationState(validationData,
                    "inProcess", "none")
          ...
          $.ajax({
            url: "{{route('
                  materials.operations.transfer.validate-material-list')}}",
            method: "POST",
            ...
            success: function (e) {
              let
                  ...
                  needToShowErrorWindow = false;

              if (!validationUid) {
                materialErrorList = [];
              } else {
                delete (materialErrorList[
                    "common"]);
              }
              e.validationResult.forEach((validationElement
                ...
                if (
                  ...
                  materialListTimestamp = new Date(materialErrorList[
                  validationElement.validationUid].timestamp);
                  let currentResponseTimestamp = new Date(e.timestamp);

                  if (
                  materialListTimestamp < currentResponseTimestamp) {
                    delete (
                    materialErrorList[validationElement.
                    validationUid]);
                  } else {
                    return;
                  }
                ...
                if (validationElement.isValid)
                      {
                      updateRowsValidationState(
                validatedData, "validated",
            "valid");
                } else
      {
                  materialErrorList[validationElement.validationUid] = {};
                  materialErrorList[validationElement.validationUid].errorList =
        validationElement.
        errorList;
                  materialErrorList[validationElement.validationUid].
        timestamp = e.timestamp;
                  updateRowsValidationState(validatedData,
          "validated", "invalid");
                }
            ...
            needToShowErrorWindow = true;
              ...
              if (
        needToShowErrorWindow) {
                showErrorWindow(
          materialErrorList);
                setButtonIndicatorVisibleState(
          "createTransferOperation", false)
                setElementsDisabledState(
          false);
              }
    ...
    needToShowErrorWindow) {
      ...
      saveEditedData) {
        ...
        a();
          ...
          "При проверке данных произошла неизвестная ошибка",
          "error", 5000)
              setButtonIndicatorVisibleState("createTransferOperation",
        ...
        function updateCommonValidationState() {
          if (materialErrorList["common"]) {
            materialErrorList["common"].errorList.forEach((
              item) => {
              if (item.
                type ===
                  "totalWeightIsTooLarge") {
                let summary = $(".
                    computed-weight-total-summary");
                $('<i/>').addClass("dx-link
                  angle")
                  .attr("style",
                  "color: #ffd358; margin-right: 4px;")
                  .attr('severity', item.
                    severity)
                  .click((e) => {
                    e.preventDefault();
        ...
        function updateRowsValidationState
                (data, validationState, validationResult, validationDiv) {
    ...
    validationState: validationState, val
      lt: validationResult })
              .done((dataObj, key) => {
                let
        validationIndicatorDiv;
                if (validationDiv) {
                  validationIndicatorDiv = validationDiv;
                } else {
                  validationIndicatorDiv = $('[validation-uid=' + dataObj.
        validationUid + ']');
                }
        ...
        validationState) {
                  case "inProcess":
                    let
        indicatorDiv = $('<div class="cell-validation-loading-indicator"
        >');
        ...
        checkIcon = $("<i/>")
                        .addClass(
          "dx-link dx-icon fas fa-check-circle dx-link-icon")
                        .attr("style",
          "color: #8bc34a")
          ...
          appendTo(validationIndicatorDiv);
                      ...
                      if (!materialErrorList
            [element.
          validationUid].errorList) {
                        return;
                      }

                      let errorList = materialErrorList[
        element.
        validationUid].errorList;
                      let maxSeverity = 0;
                      let er
      ...
      errorItem.severity > maxSeverity) {
                          maxSeverity = errorItem.
        severity;
                        }
        errorDescription = errorDescription + "<li>" + errorItem.message + "
      </li>"
                      })
        ...
        ';
          ...
          exclamationTriangleStyle = "display: none";
                      ...
                      exclamationTriangle.attr('style',
        e
      riangleStyle)

      ;
                      ex
        ngle.attr('severity', maxSeverity);
      ...
      null
            ...
            success: function (data, textStatus, jqXHR) {
              if (transferOperationInitiator === "none" || transferOperationInitiator
    === "source") {
                window.location.href = '{{route('
      materials.index')}}/?project_object=' + sourceProjectObjectId
              }
              if (transferOperationInitiator === "destination") {
                window.location.href = '{{route('

    materials.index')}}/?project_object=' + desti
      ctObjectId
              }
            },
        ...
        function
          ...
          recalculateStandardsRemains(editedRowKey) {
          transferMaterialStore.byKey(editedRowKey)
            .done(
          function (dataItem) {
              let calculatedQuantity = dataItem.total_quantity * dataItem.total_amount;
              let
            calculatedAmount = dataItem.
          total_amount
          ;
              let i
      ntId =

      dataItem.initial_comment_id ? dataItem.initial_comment_id : null;

              transferMaterialDataSource.store().
      createQuery().toArray().forEach((item) => {
      ...
      itemCommentId
        ===
          initialCommentId) {
                        calculatedAmount =
          Math.round((
            calculatedAmount - item.amount) * 100) / 100;
                      }
                      break;
                    default:
                      if (itemCommentId ===
            ...
            calculatedQuantity = Math.round((calculatedQuantity - item.quantity * item
          .amount) * 100
        ) / 100;
                      }
                  }
    ...
    dataItem.accounting_type}'][
      standard-id='${dataItem.

      standard_id}'][standard-quantity='${dataItem.

    quantity}'][initial-comment-id='${
dataItem.i
        itial_comment_id}']`).each(function () {
        ...
        $(".file-uploader").each(function () {
          ...
          $(this).dxFileUploader({
            ...
            onDropZoneEnter: function (e) {
              if (e.dropZoneElement.id === "dropzone-external-" + uploaderIndex){toggleDropZoneActive(e.dropZoneElement, true);
}
            },
            onDropZoneLeave: function (e) {
              if (e.dropZoneElement.id === "dropzone-external-" + uploaderIndex){toggleDropZoneActive(e.dropZoneElement, false);
}
            },
