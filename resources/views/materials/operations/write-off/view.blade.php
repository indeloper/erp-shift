...
@section('title', 'Cписание #'.json_decode($operationData)->id. ' [' .$operationRouteStage.']')
...
<script>
  $(function () {
    ...
    @endIf
        ...
        //<editor-fold desc="JS: Columns de
    >
    ...
    iguration">
      ...
      summaryType: 'sum',
        ...
        otal-summary",
                customizeText: function (data) {
                  return `Итого: ${Math.round(data.value * 1000) /
            1000} т.`;
                },
              },
            ],
          ...
          onEditorPreparing: (e) => {
            if (e.
          dataField === 'quantity' && e.
    parentType ===
    'dataRow') {
              if (e.row.data.accounting_type === 2) {
                e.cancel = true;
                e.editorElement.append
      ($(`<div>${e.row.data.
      quantity} ${e.
      row.data.
        ...
        let
          ...
          items: [
            ...
            O
              ...
              //         type: "required",
                //         messag
                жения" обязательно для заполнения'
                //     }]
                //
                // },
                {
              ...
              items: [
                {
                  ...
                  editorOptions:
                    writeOffMaterialGridConfiguration,
                },
              ],
            },
            {
                    ...
                    columns: [
                      {
                        ...
                        cellTemplate: (container, options) => {
                          console.log(
                        'fileHistoryGrid options', options);
                          let photoUrl = '';

                          if (options.data.data[0].
                        image) {
                  photoUrl = `{{ asset('
                    storage/img/user_images') }}/` + options.data.data[0].image;
                          } else {
                            photoUrl = `{{ mix('
                        img/user-male-black-shape.png') }}`;
                          }

                          let authorName = options.data.data[0].last_name +
                            ' ' +
                      options.data.data[0].first_name
                    .substring(0, 1) +
                    '. ' +
                            options.data.data[0].patronymic.
                    substring(0
                  , 1) +
            '.';

                          let
        commentDate = new Intl.DateTimeFormat('ru-RU', {
                            dateStyle: 'short',
                            timeStyle: 'short',
                          }).
    format(new Date(options
    .data.data[0].
      ...
      comment-user-name">` +
        ...
        container, options) => {
          ...
          '/' + item.file_path + item.file_name;
                            ...
                            $(`<div><a href="${imageUrl}" target="_blank">${item
          .file_type_name}</a></div>`).appendTo(container);
                          });
                        },
                      },
                      {
    ...
    $routeStageId, [77, 79]) && ($allowEditing ||
        $allowCancelling))
    ...
    uploadedFiles.push(item.uploadedFileId);
          ...
          writeOffOperationData.new_comment =
          operationForm.option(
          'formData').
            new_comment;
          writeOffOperationData.uploaded_files =
            uploadedFiles;
        ...
        function
          ...
          $.ajax({
            url: "{{
        route('materials.operations.write-off.move')}}",
            method: 'POST',
            headers: {
      ...
      'При сохранении данных произошла ошибка', 'error', 5000);
        ...
        function setElementsDisabledState(state) {
            @if(in_array(
        $routeStageId, [77,
          79]) && ($allowEditing || $allowCancelling))
            @if(

        $allowCancelling)
            $('#cancelOperationButton').dxButton(
          'instance').option('disabled', state);
            @endIf
            @if (
        $allowEditing)
            $('#applyOperationButton').dxButton('instance').
    ...
    'newCommentTextArea').option('disabled', state);
        }

        function setButtonIndicatorVisibleState(
    buttonId,
      ...
      state) {
          if (buttonId === '*') {
            $
      ('.
      button-loading-indicator').each((index,
        element) => {
              console.log(element);
              try {
                let loadingIndicator = $(

          element).dxLoadIndicator('instance');
                if (loadingIndicator) {
            ...
            let
              loadingIndicator = $('#' + buttonId).find('.
              button-loading-indicator').dxLoadIndicator(
            'instance');
              loadingIndicator.option('visible', state);
          }
        }
        ...
        function
          ...
          $.ajax({
            ...
            data: {
              data:
              JSON.
              stringify(cancelledOperationData),
              options: null,
            },
              success: function (data, textStatus, jqXHR) {
              window.

            location.href = '{{route('materials.index')}}/?project_object=' + projectObject;
            },
            error: function
        ...
        function getWriteOffMaterialGrid() {
              return operationForm.getEditor('w
              ');
        }
          ...
          files.forEach((item) => {
            ...
            optionElement.template = (() => {
              ...
              if (item.dropzoneRole ===
              'addNewFileUploader') {
                let addDropzoneDiv = $('<div id="
              dropzone-external-add" class="
              dx-uploader-flex-box dx-theme-border-color dropzone-external">' +
                  '<div id="
                dropzone-text-add" class="
                  dx-uploader-flex-box dropzone-text">' +
                  '<i class="
                fas fa-plus dx-uploader-icon">
              </i
              >' +
                  '</div>' +
                  '</div>');
                addDropzoneDiv.click( (
                  ...
                  createFileUploaderElem
                nt();
                });
              return addDropzoneDiv;
              } else {
                let imageContainerSrc =
                item.src ? item.src : '#';
                let dropzoneContainer = $('<div/>');
                let dropzoneExternalContainer = $(`<div id="
                  dropzone-external-${item.itemIndex}" class="
                  dx-uploader-flex-box dx-theme-border-color dropzone-external">`)
                  .appendTo(dropzoneContainer);

                let
                  dropzoneImageContainer = $(`<img id="
                  dropzone-image-${item.itemIndex}" class="
                dropzone-image" src="${imageContainerSrc}" alt="" style="
                display:none"/>`)
                  .
                ...
                let dropzoneTextContainer = $
                (`<div id="dropzone-text-${item.itemIndex}" class="dx-uploader-flex-box dropzone-text">` +
                  `
                <span class="
              dx-uploader-span">Выберите файл для загрузки</span>` +
                  `</div>`)
                  .appendTo(dropzoneExternalContainer);

                if (
              imageContainerSrc !== '#') {
                  dropzoneTextContainer.attr('style',
                'display:none');
                }

                let

            uploadProgressBarContainer = $(`<div id="upload-progress-${item.itemIndex}" class="
              upload-progress
              "/>`)
                  .appendTo(
              dropzoneExternalContainer,
              );

                let fileUploader = $(`<div class="

            file-uploader" purpose="custom" index="${item.itemIndex}">` +
                  `
                  ...
                  uploadHeaders:
            {
              ...
              'content'),
                  ...
                  uploadUrl: "{{
              route('
                materials.operations.upload-file'
              )}}",
                  uploadCustomData: { uploadPurpose: fileUploader.
    ...
    onDropZoneEnter: function (e) {
      ...
      dropZoneElement.id === "d
      ernal-" + item.itemIndex) {
      toggleDropZoneActive(e.dropZoneElement,

      true);
},
                  },
                  onDropZoneLeave: function (e) {
                    if (e.dropZoneElement.id === 'dropzone-external-' + item.itemIndex) {toggleDropZoneActive(e.dropZoneElement,
    false)
