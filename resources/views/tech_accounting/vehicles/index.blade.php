                ...
                <div class="fixed-table-toolbar toolbar-for-btn" style="padding-left:0">
                    <div class="row">
                        ...
                        <div class="col-sm-6 col-md-8 text-right mt-10__mobile">
                            @can('tech_acc_vehicle_category_create')
...
{{--                                    </td>--}}
                                <td data-label="Категория">
                                    <a :href="'{{ route('building::vehicles::vehicle_categories.show', '') }}' + '/' + category.id" class="tech-link">@{{ category.name }}</a>
                                </td>
                                <td class="text-center" data-label="Баланс, шт">
                                    <a :href="'{{ route('building::vehicles::vehicle_categories.our_vehicles.index', 'category_id') }}'.split('category_id').join(category.id)" class="tech-link fw-500">@{{ category.vehicles_count }}</a>
                                </td>
                                <td class="text-right actions">
                                    <a rel="tooltip" data-original-title="Просмотр категории" :href="'{{ route('building::vehicles::vehicle_categories.show', '')  }}' + '/' + category.id" class="btn btn-link btn-xs btn-space actions btn-primary mn-0">
                                        <i class="fa fa-eye"></i>
                                    </a>
                                    @can('tech_acc_vehicle_category_edit')
                                        <a rel="tooltip" data-original-title="Редактировать" :href="'{{ route('building::vehicles::vehicle_categories.edit', 'category_id') }}'.split('category_id').join(category.id)" class="btn btn-link btn-xs btn-space actions btn-success mn-0">
                                            <i class="fa fa-edit"></i>
                                        </a>
                                    @endcan
                                    @can('tech_acc_vehicle_category_destroy')
                                        <button rel="tooltip" data-original-title="Удалить" class="btn btn-link btn-xs btn-space btn-danger mn-0" @click="deleteCategory(category.id)">
