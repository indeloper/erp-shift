                    ...
                    <div id="cat_options">
                        <template>
                          ...
                          <el-table :data="tableData" :default-sort="{order: 'ascending'}" style="width: 100%" >
                            ...
                            <el-table-column label="Действия" min-width="70" v-if="hideMobile" header-align="center">
                              <template slot-scope="scope">
...
@section('js_footer')
    ...
    var editable = new Vue({
        ...
        data: {
            observer_key: 1,
            cat_name: '{!! addslashes($vehicle_category->name) !!}',
            cat_description: {!! json_encode($vehicle_category->description) !!} ?
                '{!! json_encode($vehicle_category->description) !!}'.slice(1).slice(0, '{!! json_encode($vehicle_category->description) !!}'.length - 2) :
                '',
            tableData: {!! json_encode($vehicle_category->characteristics ?? []) !!},
        ...
        methods: {
            ...
            deleteRow(index, rows) {
                if (this.tableData[index]['id'] !== -1) this.deleted_characteristic_ids.push(this.tableData[index]['id']);
            ...
            submitForm() {
                this.$refs.observer.validate().then(success => {
                    ...
                    axios.put('/building/vehicles/vehicle_categories/{{ $vehicle_category->id }}', {
                        deleted_characteristic_ids: this.deleted_characteristic_ids,
