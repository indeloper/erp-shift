        ...
        <div class="col-md-12 mobile-card">
            ...
            <div class="card col-xl-10 mr-auto ml-auto pd-0-min">
                <div class="card-body card-body-tech">
                    <h4 class="h4-tech fw-500" style="margin-top:0">Редактирование категории</h4>
                    ...
                    <div id="cat_options">
                        <template>
                            ...
                            <el-table :data="tableData" :default-sort="{order: 'ascending'}" style="width: 100%" >
                                ...
                                <el-table-column prop="is_hidden" label="Обязательно" header-align="center" min-width="100" v-if="hideMobile">
                                    <template slot-scope="scope">
                                        <el-checkbox name="required" v-model="scope.row.required"></el-checkbox>
                                    </template>
                                </el-table-column>
...
@section('js_footer')
    <script>
        var category = new Vue({
            ...
            data() {
                return {
                    cat_name: '{!! addslashes( $technic_category->name) !!}',
                    cat_description: {!! json_encode($technic_category->description) !!} ?
                                    '{!! json_encode($technic_category->description) !!}'.slice(1).slice(0, '{!! json_encode($technic_category->description) !!}'.length - 2) :
                                    '',
                }
            }
        });
        ...
        var characteristics = new Vue({
            el:'#cat_options',
            data: {
                tableData: {!! json_encode($technic_category->category_characteristics ?? []) !!},
            ...
            methods: {
                deleteRow(index, rows) {
                    this.deleted_characteristic_ids.push(this.tableData[index]['id']);
                ...
                submitForm() {
                    ...
                    if (formError) {
                        ...
                        axios.put('/building/tech_acc/technic_category/{{ $technic_category->id }}', {
                            deleted_characteristic_ids: characteristics.deleted_characteristic_ids,
