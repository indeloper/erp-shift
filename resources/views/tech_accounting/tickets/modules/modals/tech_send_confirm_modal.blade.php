<div class="modal fade bd-example-modal-lg show" id="tech-send-confirm" role="dialog" aria-labelledby="modal-search" style="display: none;">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content pb-3">
            <div class="modal-header">
                <h5 class="modal-title">Фиксация отправки техники</h5>
            ...
            <div class="modal-body pt-0">
                <validation-observer ref="observer" :key="observer_key">
                    <div class="modal-section mt-3">
                        ...
                        <validation-provider rules="max:300" vid="send-confirm-comment-input"
                                             ref="send-confirm-comment-input" v-slot="v">
                            <el-input
                                :class="v.classes"
                                type="textarea"
                                :rows="4"
                                maxlength="300"
                                id="send-confirm-comment-input"
                                clearable
                    ...
                    <div class="modal-section mt-3" id="back-upload-section-send">
                    ...
                    <div class="modal-section mt-3" id="front-upload-section-send">
                        ...
                        <div class="error-message d-none" id="front-upload-section-send-error">Обязательное поле</div>
                    </div>
                    <div class="modal-section mt-3" id="doc-upload-section-send">
                        ...
                        <div class="error-message d-none" id="doc-upload-section-send-error">Обязательное поле</div>
                    </div>
                    <div class="modal-section text-center mt-30">
                        <button :class="{'btn btn-success btn-sm' : window_width > 769, 'btn btn-success w-100' : window_width <= 769}" @click.stop="submit">Подтвердить отправку</button>
                    </div>
                </validation-observer>
            </div>
        </div>
...
@push('js_footer')
<script>
    var techSendConfirm = new Vue({
        el: '#tech-send-confirm',
        data: {
            observer_key: 1,
            comment: '',
        ...
        methods: {
            ...
            handleFrontSuccess(response, file, fileList) {
                ...
                $('#front-upload-section-send').removeClass('failed');
                $('#front-upload-section-send-error').addClass('d-none');
            },
            handleBackSuccess(response, file, fileList) {
                ...
                $('#back-upload-section-send-error').addClass('d-none');
            },
            handleDocSuccess(response, file, fileList) {
                ...
                $('#doc-upload-section-send-error').addClass('d-none');
            },
            ...
            submit() {
                this.$refs.observer.validate().then(success => {
                    ...
                    if (this.files_uploaded_front.length > 0) {
                        ...
                        $('#front-upload-section-send').addClass('failed');
                        $('#front-upload-section-send-error').removeClass('d-none');
                        error = true;
                    }
                    if (this.files_uploaded_back.length > 0) {
                        $('#back-upload-section-send').removeClass('failed');
                        $('#back-upload-section-send-error').addClass('d-none');
                    } else {
                        $('#back-upload-section-send').addClass('failed');
                        $('#back-upload-section-send-error').removeClass('d-none');
                        error = true;
                    }
                    if (this.files_uploaded_doc.length > 0) {
                        $('#doc-upload-section-send').removeClass('failed');
                        $('#doc-upload-section-send-error').addClass('d-none');
                    } else {
                        $('#doc-upload-section-send').addClass('failed');
                        $('#doc-upload-section-send-error').removeClass('d-none');
                        error = true;
                    }
                    if (error) {
                        return;
