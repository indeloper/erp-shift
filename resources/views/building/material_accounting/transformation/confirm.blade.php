                        ...
                        <div class="row">
                            ...
                            <div class="card-body">
                                ...
                                <div id="materials_to">
                                    <div
                                    is="material-item-to"
                                    v-for="(material_input, index) in material_inputs":index="index"
                                    :key="index + '-' + material_input.id"
                                    :material_index="material_input.id"
                                    :material_id.sync="material_input.material_id"
                                    :material_unit.sync="material_input.material_unit"
                                    :material_count.sync="material_input.material_count"
                                    :materials.sync="material_input.materials":used.sync="material_input.used"
                                    :units.sync="material_input.units"
                                    v-on:remove="material_inputs.splice(index, 1)"
                                    :inputs_length="material_inputs.length">
                                            </div>
                                ...
                                <div class="row" style="margin-top:20px">
                                    ...
                                    <div class="row" style="margin-top:20px">
                                        <div class="col-md-12 text-left">
                                            <button type="button" @click="sendAndAdd"
                                                    class="btn btn-wd btn-info btn-outline">Подтвердить и дополнить
                                            </button>
                                        </div>
                                    </div>
                                </div>
          ...
          <div class="col-md-2">\
            ...
            <template>\
                <el-checkbox v-model="default_material_used"\
                    border class="d-block" disabled></el-checkbox>\
                   </template>\
                  </div>\
                  <div class="text-center col-md-12" v-if="inputs_length > 1">\
                    <button rel="tooltip" type="button" v-on:click="$emit(\'remove\')" class="btn-remove-big">\
                        <i style="font-size:18px;" class="fa fa-times remove-stroke"></i>\
                    </button>\
                  </div>\
                  </div>\
                  ',
                    props: ['material_id', 'material_unit', 'material_count', 'inputs_length', 'material_index', 'used', 'materials', 'units', 'index', 'used'],
                    computed: {
          ...
          <div class="col-md-2">\
            ...
            <template>\
                <el-checkbox v-model="default_material_used"\
                  ...
                  <div class="col-md-12 text-center" v-if="inputs_length > 1">\
            ...
            ',
                    props: ['material_id', 'material_unit', 'material_count', 'inputs_length', 'material_index', 'used', 'materials', 'units'],
                    methods: {
                        changeMaterialId(value) {
    ...
    methods: {
        ...
        send() {
            swal({
                ...
                if (result.value) {
            axios.post('{{ route('building::mat_acc::transformation::accept', $operation->id) }}', {
                materials_from: materials_from.material_inputs,
                materials_to: materials_to.material_inputs,
                comment: answer.comment}).then(function (response) {
        ...
        sendAndAdd() {
            axios.post('{{ route('building::mat_acc::' . $operation->english_type_name . '::accept', $operation->id) }}', {
                materials_from: materials_from.material_inputs,
                materials_to: materials_to.material_inputs,
                comment: answer.comment}).then(function (response) {
                if (!response.data.message) {
